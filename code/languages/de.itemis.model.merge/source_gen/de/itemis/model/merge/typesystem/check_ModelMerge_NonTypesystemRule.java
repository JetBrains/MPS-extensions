...
import de.itemis.model.merge.runtime.runtime.MergePolicyConflict;
import de.itemis.model.merge.runtime.runtime.MissingMergePolicies;
...
public class check_ModelMerge_NonTypesystemRule extends AbstractNonTypesystemRule_Runtime implements NonTypesystemRule_Runtime {
  ...
  public void applyRule(final SNode modelMerge, final TypeCheckingContext typeCheckingContext, IsApplicableStatus status) {
    try {
      MergerResolverImpl mergerResolver = MergeResolverFromModelMerge.run(modelMerge);
      ...
      _FunctionTypes._return_P1_E0<? extends HashSet<SNode>, ? super SNode> conceptToRelevantProperties = (SNode acd) -> {
        ...
        return new HashSet<SNode>(ListSequence.fromList(AbstractConceptDeclaration__BehaviorDescriptor.getPropertyDeclarations_idhEwILLM.invoke(acd)).where(new IWhereFilter<SNode>() {
          public boolean accept(final SNode it) {
            return !(CollectionSequence.fromCollection(forbiddenProperties).any(new IWhereFilter<SProperty>() {
              public boolean accept(SProperty prop) {
                return (boolean) PropertyDeclaration__BehaviorDescriptor.is_id4MKjpUYnih4.invoke(it, prop);
              }
            }));
          }
        }).toListSequence());
      ...
      CheckinRuleUtil.warn(ModelMerge__BehaviorDescriptor.languageConcepts_id3xJ_LYXlmVz.invoke(modelMerge), repository, mergerResolver.conceptToProperty(), conceptToRelevantProperties, (SAbstractConcept sac, SNode pd) -> {
        {
      ...
      _FunctionTypes._return_P1_E0<? extends HashSet<SNode>, ? super SNode> conceptToRelevantChildren = (SNode acd) -> {
        ...
        return new HashSet<SNode>(ListSequence.fromList(AbstractConceptDeclaration__BehaviorDescriptor.getLinkDeclarations_idhEwILKK.invoke(acd)).where(new IWhereFilter<SNode>() {
          public boolean accept(SNode it) {
            return !(SEnumOperations.isMember(SPropertyOperations.getEnum(it, PROPS.metaClass$PeKc), 0xfc6f4e95b8L));
          }
        }).where(new IWhereFilter<SNode>() {
          public boolean accept(final SNode it) {
            return !(CollectionSequence.fromCollection(forbiddenChildren).any(new IWhereFilter<SContainmentLink>() {
              public boolean accept(SContainmentLink child) {
                return (boolean) LinkDeclaration__BehaviorDescriptor.is_id4MKjpUYniHA.invoke(it, child);
              }
            }));
          }
        }).toListSequence());
      };
      CheckinRuleUtil.warn(ModelMerge__BehaviorDescriptor.languageConcepts_id3xJ_LYXlmVz.invoke(modelMerge), repository, mergerResolver.conceptToChildLink(), conceptToRelevantChildren, (SAbstractConcept sac, SNode ld) -> {
        {
          final MessageTarget errorTarget = new NodeMessageTarget();
      ...
      CheckinRuleUtil.warn(ModelMerge__BehaviorDescriptor.languageConcepts_id3xJ_LYXlmVz.invoke(modelMerge), repository, mergerResolver.conceptToReferenceLink(), conceptToRelevantReferences, (SAbstractConcept sac, SNode ld) -> {
        {
          final MessageTarget errorTarget = new NodeMessageTarget();
      ...
      Map<SAbstractConcept, Set<SAbstractConcept>> isCovered = ImmutableMap.copyOf(SetSequence.fromSet(conceptsWithId).select(new ISelector<SAbstractConcept, ImmutablePair<SAbstractConcept, Set<SAbstractConcept>>>() {
        public ImmutablePair<SAbstractConcept, Set<SAbstractConcept>> select(SAbstractConcept it) {
          return ImmutablePair.of(it, Collections.singleton(it));
        }
      }));
      ...
      CheckinRuleUtil.warn(ModelMerge__BehaviorDescriptor.languageConcepts_id3xJ_LYXlmVz.invoke(modelMerge), repository, isCovered, mustBeCovered, (SAbstractConcept sac, SAbstractConcept id) -> {
        {
          final MessageTarget errorTarget = new NodeMessageTarget();
    ...
    } catch (MissingMergePolicies mmp) {
      String msg = "MergePolicies missing for concepts: " + CollectionSequence.fromCollection(mmp.missingPoliciesFor()).select(new ISelector<SNode, String>() {
        public String select(SNode it) {
          return AbstractConceptDeclaration__BehaviorDescriptor.getPresentation_id280s3ZNTXNS.invoke(it) + " ";
        }
      });
      {
        final MessageTarget errorTarget = new NodeMessageTarget();
