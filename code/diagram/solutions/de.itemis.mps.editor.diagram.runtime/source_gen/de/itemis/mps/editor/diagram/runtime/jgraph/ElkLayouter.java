...
public abstract class ElkLayouter implements ILayouter {
  private static final Logger LOG = Logger.getLogger(ElkLayouter.class);
  ...
  public void layout(mxGraph model, @Nullable ProgressIndicator progress) {
    ...
    ApplicationManager.getApplication().invokeAndWait(() -> {
      ...
      try {
        ...
        LogContext.with(ElkLayouter.class, ex, null).error("Layouting failed");
      } finally {
  ...
  protected ElkNode buildElementIfRequired(BaseDCell dcell) {
    ElkNode knode = MapSequence.fromMap(dcell2knode).get(dcell);
    if (knode == null) {
      knode = buildElement(dcell);
      MapSequence.fromMap(dcell2knode).put(dcell, knode);
    }
    return knode;
  }

  protected ElkNode buildElement(BaseDCell dcell) {
    ElkNode knode = ElkGraphUtil.createNode(null);
    if (dcell.getParent() instanceof BaseDCell) {
      ElkNode parent = buildElementIfRequired(((BaseDCell) dcell.getParent()));
    ...
    if (dcell instanceof BoxDCell) {
      ...
      addLayoutInformationToSubdiagram(knode, ((SubDiagramDCell) dcell));
  ...
  protected ElkNode buildKGraph() {
    ...
    Sequence.fromIterable(allMxCells).ofType(BoxDCell.class).visitAll(new IVisitor<BoxDCell>() {
      public void visit(BoxDCell it) {
        MapSequence.fromMap(box2dcell).put(it.getBox(), it);
      }
    });

    ElkNode graph = buildElementIfRequired(((RootDCell) myGraph.getDefaultParent()));
    addLayoutInformationToGraph(graph);

    for (BoxDCell box : Sequence.fromIterable(allMxCells).ofType(BoxDCell.class)) {
      buildElementIfRequired(box);
    }
    ...
    for (EdgeDCell edge : Sequence.fromIterable(allMxCells).ofType(EdgeDCell.class)) {
      IConnectionEndpoint_Internal resolvedEndpointFrom = check_v3bawt_a0a0t0je(edge.getEdge());
      IConnectionEndpoint_Internal resolvedEndpointTo = check_v3bawt_a0b0t0je(edge.getEdge());
      ...
      if (canLayoutLabels()) {
        for (EdgeLabelType type : EdgeLabelType.values()) {
          EditorCell labelCell = check_v3bawt_a0a0a0mb0t0je(edge.getEdge(), type);
          if (labelCell != null) {
  ...
  protected void addLayoutInformationToNode(ElkNode knode, BoxDCell box) {
    ...
    Tuples._2<Double, Double> minSize = ListSequence.fromList(box.getMPSCells()).select(new ISelector<MPSCell, Tuples._2<Double, Double>>() {
      public Tuples._2<Double, Double> select(MPSCell it) {
        return it.getMinSize();
      }
    }).first();
    if (minSize != null) {
    ...
    Bounds subdiagramBounds = Sequence.fromIterable(subdiagrams).select(new ISelector<SubDiagramDCell, Bounds>() {
      public Bounds select(SubDiagramDCell it) {
        return CellExtensions.getBounds(it);
      }
    }).reduceLeft(new ILeftCombinator<Bounds, Bounds>() {
      public Bounds combine(Bounds a, Bounds b) {
        return ExtensionMethods.toBounds(ExtensionMethods.toRectangle2D(a).createUnion(ExtensionMethods.toRectangle2D(b)));
      }
    });
    ...
    if (subdiagramBounds != null) {
      ...
      if (top < 0 || left < 0 || right < 0 || bottom < 0) {
        if (LOG.isWarningLevel()) {
          LOG.warning("Negative padding: " + calculatedPadding);
        }
      } else {
  ...
  protected void addLayoutInformationToEdgeLabel(ElkEdge kedge, ElkLabel klabel, EdgeDCell edge, EdgeLabelType type) {
    IAccessorKey ID = edge.getEdge().getAccessor().getId();
    EditorCell labelCell = check_v3bawt_a0b0cf(edge.getEdge(), type);
  ...
  protected synchronized void layoutToModel() {
    ...
    if (myWriteBackEdges) {
      ...
      for (IMapping<ElkLabel, Tuples._2<EdgeDCell, EdgeLabelType>> entry : MapSequence.fromMap(labelEdgeMap)) {
        ...
        {
          Tuples._2<EdgeDCell, EdgeLabelType> _tmp_v3bawt_d0c0c0if = entry.value();
          edge = _tmp_v3bawt_d0c0c0if._0();
          type = _tmp_v3bawt_d0c0c0if._1();
        }
        LabelDCell labelDCell = edge.getLabelCell(type);
  ...
  private static IConnectionEndpoint_Internal check_v3bawt_a0a0t0je(Edge checkedDotOperand) {
    if (null != checkedDotOperand) {
      return checkedDotOperand.getResolvedEndpointFrom();
    }
    return null;
  }
  private static IConnectionEndpoint_Internal check_v3bawt_a0b0t0je(Edge checkedDotOperand) {
    if (null != checkedDotOperand) {
      return checkedDotOperand.getResolvedEndpointTo();
    }
    return null;
  }
  private static EditorCell check_v3bawt_a0a0a0mb0t0je(Edge checkedDotOperand, EdgeLabelType type) {
    if (null != checkedDotOperand) {
      return checkedDotOperand.getLabelCell(type);
    }
    return null;
  }
  private static EditorCell check_v3bawt_a0b0cf(Edge checkedDotOperand, EdgeLabelType type) {
    if (null != checkedDotOperand) {
      return checkedDotOperand.getLabelCell(type);
    }
    return null;
