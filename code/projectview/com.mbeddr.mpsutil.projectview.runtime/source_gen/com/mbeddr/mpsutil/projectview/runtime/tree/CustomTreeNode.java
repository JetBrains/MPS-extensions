...
public class CustomTreeNode<E> extends MPSTreeNode implements ITreeNode {
  ...
  protected void doUpdatePresentation() {
    ...
    tree.getUpdater().recordPresentationLoad(this, () -> {
      ...
      if (myElement.getObject() instanceof SNode) {
        setAutoExpandable(!(isRootSNode(as_fdekl4_a0a0a0a6a1a0d0u(myElement.getObject(), SNode.class))));
  ...
  protected List<ViewElement> queryChildElements() {
    ...
    project.getRepository().getModelAccess().runReadAction(() -> {
      for (IViewElementDescriptor childDescriptor : Sequence.fromIterable(getChildDescriptors())) {
        ...
        Sequence.fromIterable(elements).visitAll(new IVisitor<ViewElement<Object>>() {
          public void visit(ViewElement<Object> it) {
            it.setParent(myElement);
          }
        });
  ...
  protected void loadChildElements(final List<ViewElement> queryResult) {
    myElement.getProject().getRepository().getModelAccess().runReadAction(() -> {
      removeAllChildren();
      FolderStructureBuilder.build(ListSequence.fromList(queryResult).select(new ISelector<ViewElement, CustomTreeNode>() {
        public CustomTreeNode select(ViewElement it) {
          return new CustomTreeNode(it);
        }
      }).toListSequence(), CustomTreeNode.this);
      myInitialized = true;
      setInvalid(false);
      if (isInTree()) {
        (getTree().getModel()).nodeStructureChanged(CustomTreeNode.this);
      }
    });
  }
  ...
  public boolean isInvalid() {
    return myIsInvalid || check_fdekl4_a0a0sc(as_fdekl4_a0a0a0sc(getParent(), ITreeNode.class), this);
  }
  ...
  public boolean canUpdateManually() {
    return isInvalid() && !(check_fdekl4_a0a0a47(as_fdekl4_a0a0a0a47(getParent(), ITreeNode.class), this));
  }
  ...
  private static <E> boolean check_fdekl4_a0a0sc(ITreeNode checkedDotOperand, CustomTreeNode<E> checkedDotThisExpression) {
    if (null != checkedDotOperand) {
      return checkedDotOperand.isInvalid();
    }
    return false;
  }
  private static <E> boolean check_fdekl4_a0a0a47(ITreeNode checkedDotOperand, CustomTreeNode<E> checkedDotThisExpression) {
    if (null != checkedDotOperand) {
      return checkedDotOperand.isInvalid();
    }
    return false;
  }
  private static <T> T as_fdekl4_a0a0a0a6a1a0d0u(Object o, Class<T> type) {
    return (type.isInstance(o) ? (T) o : null);
  }
  private static <T> T as_fdekl4_a0a0a0sc(Object o, Class<T> type) {
    return (type.isInstance(o) ? (T) o : null);
  }
  private static <T> T as_fdekl4_a0a0a0a47(Object o, Class<T> type) {
    return (type.isInstance(o) ? (T) o : null);
  }
}
