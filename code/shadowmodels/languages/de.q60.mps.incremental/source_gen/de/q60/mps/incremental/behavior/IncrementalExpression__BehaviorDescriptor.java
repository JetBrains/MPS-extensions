...
public final class IncrementalExpression__BehaviorDescriptor extends BaseBHDescriptor {
  ...
  /*package*/ static String getCacheKeyClassName_id7qGGLAkzosl(@NotNull SNode __thisNode__) {
    ...
    Iterable<SNode> inSameNamespace = ListSequence.fromList(SNodeOperations.getNodeDescendants(namespaceNode, CONCEPTS.IncrementalExpression$yl, false, new SAbstractConcept[]{})).where(new IWhereFilter<SNode>() {
      public boolean accept(SNode it) {
        return Objects.equals(IncrementalExpression__BehaviorDescriptor.getNamespaceNode_id7qGGLAkzPzR.invoke(it), namespaceNode);
      }
    }).toListSequence();
    String suffix = (Sequence.fromIterable(inSameNamespace).count() == 1 ? "" : "_" + Sequence.fromIterable(inSameNamespace).indexOf(__thisNode__));
    String name = "CK_" + IterableUtils.join(ListSequence.fromList(ListSequence.fromList(SNodeOperations.getNodeAncestors(__thisNode__, CONCEPTS.INamedConcept$Kd, false)).toListSequence()).reversedList().select(new ISelector<SNode, String>() {
