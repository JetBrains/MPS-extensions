...
public class NodeAsOutputNode implements IOutputNode {
  ...
  public Iterable<IOutputNodeReference> getChildren(String role) {
    ...
    return Sequence.fromIterable(children).select(new ISelector<INode, IOutputNodeReference>() {
      public IOutputNodeReference select(INode it) {
        return NodeAsOutputNodeReference.create(it);
      }
    });
  }
  @Override
  public Iterable<IOutputNodeReference> getAllChildren() {
    ...
    return Sequence.fromIterable(allChildren).select(new ISelector<INode, IOutputNodeReference>() {
      public IOutputNodeReference select(INode it) {
        return NodeAsOutputNodeReference.create(it);
      }
    });
