...
public abstract class AbstractRule implements IRule {
  ...
  protected SNode findNodeToRecycle(PNode parseTree, final IRecycledNodes recycledNodes) {
    Iterable<IToken> textTokens = ListSequence.fromList(parseTree.getChildren()).select(new ISelector<PNode, IToken>() {
      public IToken select(PNode it) {
        return it.getToken();
      }
    }).where(new IWhereFilter<IToken>() {
      public boolean accept(IToken it) {
        return check_uly4a7_a0a0a0a0a6(it);
      }
    });
    Set<SNode> candidates = SetSequence.fromSetWithValues(new HashSet<SNode>(), Sequence.fromIterable(textTokens).select(new ISelector<IToken, SNode>() {
      public SNode select(IToken it) {
        return recycledNodes.getOriginalNode(it);
      }
    }));
