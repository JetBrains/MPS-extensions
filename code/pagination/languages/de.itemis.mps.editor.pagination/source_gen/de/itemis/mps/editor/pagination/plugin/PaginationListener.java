...
public class PaginationListener extends MouseAdapter {
  ...
  public Optional<SNode> getAncestorWithLink(SNode node, final SContainmentLink link) {
    return Optional.ofNullable(ListSequence.fromList(SNodeOperations.getNodeAncestors(node, null, true)).findFirst(new IWhereFilter<SNode>() {
      public boolean accept(SNode ancestorNode) {
        return Objects.equals(SNodeOperations.getContainingLink(ancestorNode), link);
      }
    }));
  }
  ...
  public Iterable<EditorCell_Collection> findDescendantsPaginatedCells(EditorCell startingCell) {
    return Sequence.fromIterable(EditorCellUtil.getDescendants(startingCell, true)).select(new ISelector<EditorCell, EditorCell_Collection>() {
      public EditorCell_Collection select(EditorCell cell) {
        return as_jbb13b_a0a0a0a0a0a0a0bb(cell, EditorCell_Collection.class);
      }
    }).where(new NotNullWhereFilter<EditorCell_Collection>()).where(new IWhereFilter<EditorCell_Collection>() {
      public boolean accept(EditorCell_Collection cell) {
        return isPaginatedCell(cell);
      }
    });
  }
  ...
  public Iterable<PageChange> getPaginatedCellsChanges(EditorCell closestVisibleCell, final SNode targetNode) {
    return Sequence.fromIterable(findDescendantsPaginatedCells(closestVisibleCell)).select(new ISelector<EditorCell_Collection, PageChange>() {
      public PageChange select(EditorCell_Collection paginatedCell) {
        return getPaginatedCellChange(paginatedCell, targetNode);
      }
    }).where(new NotNullWhereFilter<PageChange>());
  }
        ...
        Optional.ofNullable(EditorCellUtil.openEditorComponentForNode(targetNode, project)).ifPresent((jetbrains.mps.openapi.editor.EditorComponent editorComponentForTarget) -> {
          ...
          while (pageChanged) {
            pageChanged = Optional.ofNullable(Sequence.fromIterable(EditorCellUtil.getNodesWithCell(SNodeOperations.getNodeAncestors(targetNode, null, true), editorComponentForTarget)).first()).map((EditorCell firstAncestorVisibleCell) -> {
              ...
              Sequence.fromIterable(changesToApply).visitAll(new IVisitor<PageChange>() {
                public void visit(PageChange change) {
  ...
  private static <T> T as_jbb13b_a0a0a0a0a0a0a0bb(Object o, Class<T> type) {
