...
public class MyConnectPreview extends mxConnectPreview {
  ...
  public Object stop(final boolean commit, final MouseEvent event) {
    if (commit) {
      ...
      if (ListSequence.fromList(applicableTypes).count() > 1) {
        ...
        List<AnAction> actions = ListSequence.fromList(applicableTypes).sort(new ISelector<IConnectionType_Internal, String>() {
          public String select(IConnectionType_Internal it) {
            return it.getName();
          }
        }, true).select(new ISelector<IConnectionType_Internal, AnAction>() {
          public AnAction select(final IConnectionType_Internal it) {
            return as_8ai88l_a0a0a0a0a0a0e0g0a0d(new AnAction(it.getName(), "", new ConnectionTypeIcon(it)) {
  ...
  private static <T> T as_8ai88l_a0a0a0a0a0a0e0g0a0d(Object o, Class<T> type) {
