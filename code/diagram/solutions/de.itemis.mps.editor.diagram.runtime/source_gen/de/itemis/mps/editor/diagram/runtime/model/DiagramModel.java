...
public class DiagramModel {
  ...
  private static Map<EditorComponent, Map<SNode, Map<String, DiagramModel>>> ourInstances = MapSequence.fromMap(new HashMap<EditorComponent, Map<SNode, Map<String, DiagramModel>>>());
  ...
  public List<Annotation> getAnnotations(final IDiagramElement annotatedElement) {
    return SetSequence.fromSet(myAnnotations).where(new IWhereFilter<Annotation>() {
      public boolean accept(Annotation it) {
        return it.getAnnotatedElement() == annotatedElement;
      }
    }).toListSequence();
  }
  ...
  public void removeElements(Iterable<IDiagramElement> toRemove) {
    Sequence.fromIterable(toRemove).visitAll(new IVisitor<IDiagramElement>() {
  ...
  public void clearChildModels() {
    SetSequence.fromSet(MapSequence.fromMap(myChildModels).keySet()).visitAll(new IVisitor<DiagramModel>() {
      public void visit(DiagramModel it) {
        it.setParentModel(null);
      }
    });
  ...
  public IConnectionEndpoint_Internal resolveEndpointGlobal(IConnectionEndpoint_Internal endpoint, boolean isEnd) {
    if (endpoint instanceof UnresolvedEndpoint) {
      ...
      if (!(SetSequence.fromSet(myReportedEndpoints).contains(referenceString))) {
        ...
        LOG.warn("Cannot resolve endpoint: " + referenceString);
      }
    } else {
      return endpoint;
    }
  ...
  public Iterable<DiagramModel> getDescendantModels() {
    ...
    return SetSequence.fromSet(MapSequence.fromMap(myChildModels).keySet()).concat(SetSequence.fromSet(MapSequence.fromMap(myChildModels).keySet()).translate(new ITranslator2<DiagramModel, DiagramModel>() {
      public Iterable<DiagramModel> translate(DiagramModel it) {
        return it.getDescendantModels();
      }
    }));
  }
