...
public abstract class AbstractGridLayout extends AbstractLayout implements ILayouter {
  ...
  public int getAscent(ILayoutableContainer container, Size sizeConstraint) {
    ...
    GridElement baseLineGridElement = Sequence.fromIterable(grid.getValidElements()).findFirst(new IWhereFilter<GridElement>() {
      public boolean accept(GridElement it) {
        return it.getLayoutable().isBaseLineCell();
      }
    });
    if (baseLineGridElement == null) {
      // handle DEFAULT_BASE_LINE
      final ILayoutable baselineChild = container.getBaselineChild();
      baseLineGridElement = Sequence.fromIterable(grid.getValidElements()).findFirst(new IWhereFilter<GridElement>() {
        public boolean accept(GridElement it) {
          return LayoutableExtensions.isAncestorOf(baselineChild, it.getLayoutable());
        }
      });
    }
    if (baseLineGridElement != null) {
  ...
  protected void layoutGrid(Grid grid, ILayoutableContainer container, Size sizeConstraint, boolean fit) {
    ...
    if (fit && sizeConstraint.isWidthLimited()) {
      ...
      if (availableWidth > 0) {
        Set<Integer> columnsToGrow = SetSequence.fromSetWithValues(new HashSet<Integer>(), Sequence.fromIterable(grid.getElements()).where(new IWhereFilter<GridElement>() {
          public boolean accept(GridElement it) {
            return it.getLayoutable() != null && isChildPushX(it.getLayoutable());
          }
        }).select(new ISelector<GridElement, Integer>() {
          public Integer select(GridElement it) {
            return it.getColumnIndex();
          }
        }));
    ...
    while (grid.getWidth() > sizeConstraint.getWidth() && --timeout > 0) {
      ...
      // ensure min width
      for (GridElement element : Sequence.fromIterable(grid.getValidElements()).sort(new ISelector<GridElement, Integer>() {
        public Integer select(GridElement it) {
          return it.getColumnSpan();
        }
      }, true)) {
    ...
    if (fit && sizeConstraint.isHeightLimited()) {
      ...
      if (availableHeight > 0) {
        Set<Integer> rowsToGrow = SetSequence.fromSetWithValues(new HashSet<Integer>(), Sequence.fromIterable(grid.getElements()).where(new IWhereFilter<GridElement>() {
          public boolean accept(GridElement it) {
            return it.getLayoutable() != null && isChildPushY(it.getLayoutable());
          }
        }).select(new ISelector<GridElement, Integer>() {
          public Integer select(GridElement it) {
