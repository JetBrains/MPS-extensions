...
public final class mapProperties_Intention extends AbstractIntentionDescriptor implements IntentionFactory {
  ...
  /*package*/ final class IntentionImplementation extends AbstractIntentionExecutable {
    ...
    public void execute(final SNode node, final EditorContext editorContext) {
      SNode parameterType = Sequence.fromIterable(SNodeOperations.ofConcept(SLinkOperations.collect(SLinkOperations.getChildren(SNodeOperations.getNodeAncestor(node, CONCEPTS.Transformation$Gz, false, false), LINKS.input$hSH9), LINKS.type$Dgp0), CONCEPTS.TConceptType$x0)).findFirst(new IWhereFilter<SNode>() {
        public boolean accept(SNode it) {
          return Objects.equals(SLinkOperations.getTarget(it, LINKS.concept$Czlx), SLinkOperations.getTarget(node, LINKS.concept$huZZ));
        }
      });

      Sequence.fromIterable(SLinkOperations.collectMany(AbstractConceptDeclaration__BehaviorDescriptor.getAllSuperConcepts_id2A8AB0rAWpG.invoke(SLinkOperations.getTarget(parameterType, LINKS.concept$Czlx), ((boolean) true)), LINKS.propertyDeclaration$YUgg)).visitAll(new IVisitor<SNode>() {
        public void visit(SNode it) {
          ListSequence.fromList(SLinkOperations.getChildren(node, LINKS.content$hvH2)).addElement(createPropertyBuilder_uhnc2f_a0a0a0a0c0a(it, it));
        }
      });
    }
    ...
    private boolean isApplicableToNode(final SNode node, final EditorContext editorContext) {
      return Sequence.fromIterable(SNodeOperations.ofConcept(SLinkOperations.collect(SLinkOperations.getChildren(SNodeOperations.getNodeAncestor(node, CONCEPTS.Transformation$Gz, false, false), LINKS.input$hSH9), LINKS.type$Dgp0), CONCEPTS.TConceptType$x0)).any(new IWhereFilter<SNode>() {
        public boolean accept(SNode it) {
          return Objects.equals(SLinkOperations.getTarget(it, LINKS.concept$Czlx), SLinkOperations.getTarget(node, LINKS.concept$huZZ));
        }
      });
