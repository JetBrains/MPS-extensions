...
public class RichtextSelection extends AbstractSelection {
  private static Logger LOG = Logger.wrap(LogManager.getLogger(RichtextSelection.class));
    ...
    for (int i = 0; i < selectedNodes.size(); ++i) {
      ...
      if (SNodeOperations.isInstanceOf(node, CONCEPTS.Word$5r) && (isFirst || isLast)) {
        ...
        if (removeStart > removeEnd) {
          {
            Tuples._2<Integer, Integer> _tmp_irpwvl_a0e0d0h0ob = MultiTuple.<Integer,Integer>from(removeEnd, removeStart);
            removeStart = _tmp_irpwvl_a0e0d0h0ob._0();
            removeEnd = _tmp_irpwvl_a0e0d0h0ob._1();
          }
        }
    ...
    if (ListSequence.fromList(words).count() == 1 && SNodeOperations.isInstanceOf(ListSequence.fromList(words).getElement(0), CONCEPTS.Word$5r)) {
      ...
      String plainText = IterableUtils.join(ListSequence.fromList(words).select(new ISelector<SNode, String>() {
        public String select(SNode it) {
          String text = check_irpwvl_a0a0a0a0a0a0b0qb(SNodeOperations.as(it, CONCEPTS.IWord$8d));
  ...
  public void pasteFromClipboard() {
    ...
    List<SNode> wordsFromClipboard = Sequence.fromIterable(SNodeOperations.ofConcept(nodesFromClipboard, CONCEPTS.IWord$8d)).toListSequence();
  ...
  public static RichtextSelection create(final jetbrains.mps.openapi.editor.cells.EditorCell startLeaf, final int startLeafTextPos, final jetbrains.mps.openapi.editor.cells.EditorCell endLeaf, final int endLeafCursorPos) {
    ...
    startLeaf.getEditorComponent().getEditorContext().getRepository().getModelAccess().runReadAction(new _Adapters._return_P0_E0_to_Runnable_adapter(new _FunctionTypes._return_P0_E0<RichtextSelection>() {
      public RichtextSelection invoke() {
        ...
        if (startLeaf instanceof EditorCell_Multiline || endLeaf instanceof EditorCell_Multiline) {
          EditorCell_Collection textCell = as_irpwvl_a0a0a51a0a0a0a0a2a55(editor.findNodeCell(commonParentText), EditorCell_Collection.class);

          if (leftToRight) {
            EditorCell_Word lastLeaf = as_irpwvl_a0a0a2a51a0a0a0a0a2a55(CellTraversalUtil.getLastLeaf(textCell), EditorCell_Word.class);
            endTextPos = lastLeaf.getParent().getAbsoluteTextPosition(lastLeaf, 0) + lastLeaf.getText().length();
          } else {
            EditorCell_Word firstLeaf = as_irpwvl_a0a0a0c0p0a0a0a0a0c0dc(CellTraversalUtil.getFirstLeaf(textCell), EditorCell_Word.class);
            endTextPos = 0;
          }
  ...
  public static RichtextSelection create(EditorCellLabelSelection selection) {
    EditorCell_Word wordCell = as_irpwvl_a0a0a65(check_irpwvl_a0a0a65(selection), EditorCell_Word.class);
    if (wordCell == null) {
      return null;
    }
  ...
  public static SNode getParentIWord(jetbrains.mps.openapi.editor.cells.EditorCell childCell, SNode asDirectChildOf) {
    ...
    for (SNode ancestor : ListSequence.fromList(SNodeOperations.getNodeAncestors(cellNode, null, false)).where(new IWhereFilter<SNode>() {
      public boolean accept(SNode it) {
        return SNodeOperations.isInstanceOf(it, CONCEPTS.IWord$8d);
      }
    })) {
  ...
  public static SNode getFirstCommonParentNode(jetbrains.mps.openapi.editor.cells.EditorCell child1, jetbrains.mps.openapi.editor.cells.EditorCell child2) {
    return check_irpwvl_a0a36(getFirstCommonParent(child1, child2));
  }
  ...
  private static int xCoordToCaretPos(EditorCell_Label label, int _x) {
    TextLine tl = ((TextLine) ReflectionUtil_copy.readField(EditorCell_Label.class, label, "myTextLine"));
  ...
  private static jetbrains.mps.openapi.editor.cells.EditorCell_Label check_irpwvl_a0a0a65(EditorCellLabelSelection checkedDotOperand) {
    if (null != checkedDotOperand) {
      return checkedDotOperand.getEditorCellLabel();
    }
    return null;
  }
  private static SNode check_irpwvl_a0a36(jetbrains.mps.openapi.editor.cells.EditorCell checkedDotOperand) {
    if (null != checkedDotOperand) {
  ...
  private static <T> T as_irpwvl_a0a0a51a0a0a0a0a2a55(Object o, Class<T> type) {
    return (type.isInstance(o) ? (T) o : null);
  }
  private static <T> T as_irpwvl_a0a0a2a51a0a0a0a0a2a55(Object o, Class<T> type) {
    return (type.isInstance(o) ? (T) o : null);
  }
  private static <T> T as_irpwvl_a0a0a0c0p0a0a0a0a0c0dc(Object o, Class<T> type) {
    return (type.isInstance(o) ? (T) o : null);
  }
