...
public class TransformationEngine extends UserObjectContainer implements ITransformationEngine {
  ...
  protected Iterable<ITransformationRule> selectRule(TransformationCall transformationCall, boolean allowNone) {
    ...
    if (allowNone) {
      Iterable<Object> rules = Sequence.fromIterable(this.getTransformationFunctions().callFunctions(transformationCall.getId(), transformationCall.getParameters())).toListSequence();
      ...
      try {
        ...
        RuntimeException ex2 = new RuntimeException("No transformation found for " + transformationCall.getId() + "(" + IterableUtils.join(Sequence.fromIterable(transformationCall.getParameters().asSequence()).select(new ISelector<Object, String>() {
          public String select(Object it) {
            return "" + ((it instanceof INode ? ((INode) it).getConcept() : it));
          }
        }), ", ") + ")", ex);
    ...
    List<ITransformationRule> contributions = Sequence.fromIterable(this.getTransformationFunctions().callFunctions(transformationCall.getId() + "&", transformationCall.getParameters())).select(new ISelector<Object, ITransformationRule>() {
  ...
  public Iterable<INode> executeInCurrentStage(IContainmentTarget call) {
    ...
    return Sequence.fromIterable(nodes).select(new ISelector<IOutputNodeReference, INode>() {
  ...
  public ISubgraphStage resolveStage(@NotNull IStageReference stageRef) {
    ...
    ISubgraphStage current = Sequence.fromIterable(currentlyComputedStage.getAllValues()).findLast(new IWhereFilter<ISubgraphStage>() {
      public boolean accept(ISubgraphStage it) {
        return Objects.equals(it.getStageReference(), _stageRef.value);
      }
    });
    if (current != null) {
      return current;
  ...
  public Iterable<IOutputNodeReference> resolveRootNodes(ContainmentTargetRootNodes ref) {
    ...
    if (call instanceof ISubgraphCall) {
      ...
      return Sequence.fromIterable(childStage.getRoots()).translate(new ITranslator2<IUniqueContainmentTarget, IOutputNodeReference>() {
  ...
  public Iterable<? extends Iterable<INode>> executeCallExpression(IContainmentTarget call) {
    ...
    return Sequence.<Iterable<INode>>singleton(Sequence.fromIterable(rootRefs).select(new ISelector<IOutputNodeReference, INode>() {
      public INode select(IOutputNodeReference it) {
        return OutputNodeReferenceAsNode.create(TransformationEngine.this, it);
      }
    }));
  }
  ...
  public IContainmentTarget computeRewriteReplacement(SubgraphStage stage, IUniqueContainmentTarget<RewriteCall> call) {
    ...
    if (rewriter instanceof MPSLikeRewriter) {
      ...
      List<MappingConfiguration> mcs = Sequence.fromIterable(step.getMappingConfigurations()).select(new ISelector<MappingConfigurationId, MappingConfiguration>() {
        public MappingConfiguration select(MappingConfigurationId it) {
          return MapSequence.fromMap(allMCs).get(it);
        }
      }).toListSequence();
      Iterable<ReductionRule> reductionRules = ListSequence.fromList(mcs).translate(new ITranslator2<MappingConfiguration, ReductionRule>() {
        public Iterable<ReductionRule> translate(MappingConfiguration it) {
          return it.getReductionRules();
        }
      });
...
tid:
  ...
  public Genplan getGenplan(final GoalCall goalCall) {
    return getIncrementalEngine().evaluate(new CK_TransformationEngine_getGenplan(goalCall), () -> {
      ...
      List<MappingConfiguration> mcs = ListSequence.fromList(getMappingConfigurationsList(goalCall)).toListSequence();
      List<IGenplanRule> rules = Sequence.fromIterable(TransformationEngine.this.getTransformationFunctions().callFunctions(goalId + "/rules", new ParameterList(new Object[0]))).ofType(IGenplanRule.class).toListSequence();
  ...
  public Map<MappingConfigurationId, MappingConfiguration> getMappingConfigurationsMap(final GoalCall goalCall) {
    return this.getIncrementalEngine().evaluate(new CK_TransformationEngine_getMappingConfigurationsMap(goalCall), () -> {
      ...
      ListSequence.fromList(mcs).visitAll(new IVisitor<MappingConfiguration>() {
        public void visit(MappingConfiguration it) {
          MapSequence.fromMap(map).put(it.getId(), it);
        }
      });
      return map;
    });
  ...
  public List<MappingConfiguration> getMappingConfigurationsList(final GoalCall goalCall) {
    return this.getIncrementalEngine().evaluate(new CK_TransformationEngine_getMappingConfigurationsList(goalCall), () -> {
      ...
      return Sequence.fromIterable(result).translate(new ITranslator2<Object, MappingConfiguration>() {
        public Iterable<MappingConfiguration> translate(Object it) {
          return (Iterable<MappingConfiguration>) it;
        }
      }).toListSequence();
    });
  }
  ...
  protected ISubgraphStage doFindStage(IStageReference expectedStageRef) {
    ...
    if (subgraphCall.isFixpoint()) {
      while (true) {
        ...
        List<IContainmentTarget> nextRoots = Sequence.fromIterable(currentStage.value.getRoots()).select(new ISelector<IUniqueContainmentTarget, ContainmentTargetRootNodes>() {
          public ContainmentTargetRootNodes select(IUniqueContainmentTarget it) {
            return new ContainmentTargetRootNodes(currentStage.value.getStageReference(), it);
          }
        }).translate(new ITranslator2<ContainmentTargetRootNodes, IOutputNodeReference>() {
          public Iterable<IOutputNodeReference> translate(ContainmentTargetRootNodes it) {
            return it.getNodes(TransformationEngine.this);
          }
        }).select(new ISelector<IOutputNodeReference, INode>() {
          public INode select(IOutputNodeReference it) {
            return OutputNodeReferenceAsNode.create(TransformationEngine.this, it);
          }
        }).select(new ISelector<INode, DependencyTrackingNode>() {
          public DependencyTrackingNode select(INode it) {
            return DependencyTrackingNode.wrap(it);
          }
        }).select(new ISelector<DependencyTrackingNode, IContainmentTarget>() {
          public IContainmentTarget select(DependencyTrackingNode it) {
  ...
  public void applyWeavingRules(final SubgraphStage stage) {
    ...
    Iterable<WeavingRule> weavingRules = ListSequence.fromList(mcs).translate(new ITranslator2<MappingConfiguration, WeavingRule>() {
      public Iterable<WeavingRule> translate(MappingConfiguration it) {
        return it.getWeavingRules();
      }
    });
    ...
    Iterable<INode> inputRoots = Sequence.fromIterable(stage.getRoots()).translate(new ITranslator2<IUniqueContainmentTarget, INode>() {
      public Iterable<INode> translate(IUniqueContainmentTarget it) {
        return it.getInputNodes();
      }
    });
    for (INode inputNode : Sequence.fromIterable(inputRoots).translate(new ITranslator2<INode, INode>() {
      public Iterable<INode> translate(INode it) {
        return SM_NodeUtil.getDescendants(it, true);
      }
    })) {
  ...
  public void applyPropertyRules(SubgraphStage stage) {
    List<MappingConfiguration> mcs = getMappingConfigurations(stage);
    Iterable<IOutputWrapperRule> rules = ListSequence.fromList(mcs).translate(new ITranslator2<MappingConfiguration, IOutputWrapperRule>() {
      public Iterable<IOutputWrapperRule> translate(MappingConfiguration it) {
        return it.getOutputWrapperRules();
      }
    });
    ...
    final Iterable<INode> inputRoots = Sequence.fromIterable(new StageRootNodes(stage.getStageReference()).getNodes(this)).select(new ISelector<IOutputNodeReference, OutputNodeReferenceAsNode>() {
      public OutputNodeReferenceAsNode select(IOutputNodeReference it) {
        return new OutputNodeReferenceAsNode(TransformationEngine.this, it);
      }
    });
    for (INode node : ListSequence.fromList(stage.computeWithFrozenCheckDisabled(() -> {
      return Sequence.fromIterable(inputRoots).translate(new ITranslator2<INode, INode>() {
        public Iterable<INode> translate(INode it) {
          return SM_NodeUtil.getDescendants(it, true);
        }
      }).toListSequence();
  ...
  public List<MappingConfiguration> getMappingConfigurations(SubgraphStage stage) {
    ...
    int stepNumber = stage.getStageReference().getStepNumber();
    GenplanStep step = genplan.getStep(stepNumber);
    return Sequence.fromIterable(step.getMappingConfigurations()).select(new ISelector<MappingConfigurationId, MappingConfiguration>() {
      public MappingConfiguration select(MappingConfigurationId it) {
        return MapSequence.fromMap(allMCs).get(it);
      }
    }).toListSequence();
