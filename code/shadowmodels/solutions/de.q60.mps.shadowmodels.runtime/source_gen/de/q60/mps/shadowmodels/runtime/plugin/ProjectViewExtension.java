...
public class ProjectViewExtension {
  ...
  public void attachShadowRoot() {
    ...
    DefaultTreeModel model = projectPane.getTree().getModel();
  ...
  public void attachShadowRootIfNotEmpty() {
    if (shadowTreeNode.getChildCount() == 0) {
      if (shadowTreeNode.getParent() != null && shadowTreeNode.getTree() != null) {
        shadowTreeNode.getTree().getModel().removeNodeFromParent(shadowTreeNode);
      }
    } else {
      attachShadowRoot();
    }
  ...
  public void updateModules() {
    ...
    final DefaultTreeModel treeModel = projectTree.getModel();
    if (treeModel == null) {
      return;
    }
    ...
    repository.getModelAccess().runReadAction(() -> {
      ...
      Sequence.fromIterable(getChildren(shadowTreeNode)).ofType(ShadowModuleTreeNode.class).visitAll(new IVisitor<ShadowModuleTreeNode>() {
        public void visit(ShadowModuleTreeNode it) {
          SetSequence.fromSet(treeNodesToRemove).addElement(it);
          MapSequence.fromMap(module2treeNode).put(it.getModule(), it);
        }
      });
      int insertAt = 0;
      for (SM_ShadowModule shadowModule : Sequence.fromIterable(modules).ofType(SM_ShadowModule.class).sort(new ISelector<SM_ShadowModule, String>() {
        public String select(SM_ShadowModule it) {
      ...
      SetSequence.fromSet(treeNodesToRemove).visitAll(new IVisitor<ShadowModuleTreeNode>() {
        public void visit(ShadowModuleTreeNode it) {
