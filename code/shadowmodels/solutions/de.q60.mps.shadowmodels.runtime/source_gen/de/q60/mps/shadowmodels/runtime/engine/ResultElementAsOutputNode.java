...
public class ResultElementAsOutputNode implements IOutputNode {
  ...
  public Iterable<IOutputNodeReference> getChildren(final String role) {
    return Sequence.fromIterable(getElement().getChildren(role)).translate(new ITranslator2<TransformationResult.Child, IOutputNodeReference>() {
      public Iterable<IOutputNodeReference> translate(TransformationResult.Child it) {
        return it.resolve(engine);
      }
    });
  }
  public Iterable<IOutputNodeReference> getAllChildren() {
    return Sequence.fromIterable(getElement().getAllChildren()).translate(new ITranslator2<TransformationResult.Child, IOutputNodeReference>() {
      public Iterable<IOutputNodeReference> translate(TransformationResult.Child it) {
        return it.resolve(engine);
      }
    });
