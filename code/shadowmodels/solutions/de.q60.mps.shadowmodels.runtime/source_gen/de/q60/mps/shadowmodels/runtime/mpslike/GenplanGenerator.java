...
public class GenplanGenerator {
  ...
  public Genplan generate(List<MappingConfiguration> mappingConfigurationsList, List<IGenplanRule> rules) {
    Genplan plan = new Genplan(Sequence.<GenplanStep>singleton(new GenplanStep(ListSequence.fromList(mappingConfigurationsList).select(new ISelector<MappingConfiguration, MappingConfigurationId>() {
    ...
    rules = Sequence.fromIterable(Sequence.<IGenplanRule>singleton(new GenplanRule_InputOutputLanguages())).concat(ListSequence.fromList(rules)).toListSequence();
