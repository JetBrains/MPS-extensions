...
public class BorderPainter extends AbstractAdditionalPainter {
  ...
  protected void paintBorders(Graphics2D g, EditorCell cell) {
  ...
  protected void processQueue(Graphics2D g) {
    for (PaintRectOperation op : ListSequence.fromList(myPaintQueue).sort(new ISelector<PaintRectOperation, Integer>() {
      public Integer select(PaintRectOperation it) {
        return it.getBorderWidth();
      }
    }, true).alsoSort(new ISelector<PaintRectOperation, Float>() {
      public Float select(PaintRectOperation it) {
        return it.getBrightnessPriority();
      }
    }, true)) {
      op.paint(g);
    }
