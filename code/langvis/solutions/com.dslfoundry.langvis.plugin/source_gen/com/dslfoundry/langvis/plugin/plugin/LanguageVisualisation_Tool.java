...
public class LanguageVisualisation_Tool extends GeneratedTool {
  ...
  public void rebuildBasedOnCurrentContextAndSettings() {
    if (!(new File(LanguageVisualisation_Tool.this.plantUmlJarPath).exists())) {
      ...
      LogContext.with(LanguageVisualisation_Tool.class, null, null).error(msg);
      return;
    }
    ...
    try {
      ...
      e.printStackTrace();
      LogContext.with(LanguageVisualisation_Tool.class, e, null).error("Visualizing language elements failed (rendering to PlantUML)");
    }
    ...
    LogContext.with(LanguageVisualisation_Tool.class, null, null).info("Command executed with result " + result);

    // Reload PNG
    try {
      LanguageVisualisation_Tool.this.imageViewer.loadImage(LanguageVisualisation_Tool.this.pngImagePath);
    } catch (IOException e) {
      LogContext.with(LanguageVisualisation_Tool.class, e, null).error(String.format("Could not read '%s'", LanguageVisualisation_Tool.this.pumlFilePath));
    }
