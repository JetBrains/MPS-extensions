...
public class PartialTableEditor extends AbstractTableEditor {
  private static Logger LOG = Logger.getLogger(PartialTableEditor.class);
  ...
  protected void paintContent(Graphics graphics, ParentSettings parentSettings) {
    ...
    if (settings.isSelectionPainted()) {
      ...
      cells = ListSequence.fromList(cells).select(new ISelector<EditorCell, EditorCell>() {
        public EditorCell select(EditorCell it) {
          return (it.getParent() instanceof EditorCell_GridCell ? it.getParent() : it);
        }
      }).toListSequence();
