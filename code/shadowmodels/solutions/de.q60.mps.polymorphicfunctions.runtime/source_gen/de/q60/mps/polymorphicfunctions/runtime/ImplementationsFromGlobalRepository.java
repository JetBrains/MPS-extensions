...
public abstract class ImplementationsFromGlobalRepository extends ImplementationsFromRepository {
  ...
  public synchronized Iterable<PFModule> getPFModules() {
    ...
    if (classLoaders != null) {
      if (SetSequence.fromSet(classLoaders).any(new IWhereFilter<ModuleClassLoader>() {
    ...
    if (pfModules == null) {
      MPSModuleRepository.getInstance().getModelAccess().runReadAction(() -> {
        pfModules = Sequence.fromIterable(super_getPFModules()).toListSequence();
        classLoaders = SetSequence.fromSetWithValues(new HashSet<ModuleClassLoader>(), ListSequence.fromList(pfModules).select(new ISelector<PFModule, ClassLoader>() {
          public ClassLoader select(PFModule it) {
            return it.getClass().getClassLoader();
          }
        }).ofType(ModuleClassLoader.class));
      });
    }
    return pfModules;
  }
