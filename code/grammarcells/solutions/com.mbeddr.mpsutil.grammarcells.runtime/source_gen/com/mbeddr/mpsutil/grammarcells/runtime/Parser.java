...
public class Parser implements IRecycledNodes {
  private static final Logger LOG_1855564834 = Logger.getLogger(Parser.class);
    ...
    if (newTree.getRule() != null) {
      ...
      if (ListSequence.fromList(alternativesList).count() > 1) {
        ...
        if (isNotOverriden) {
          ...
          ListSequence.fromList(alternativesList).visitAll(new IVisitor<IRule>() {
            public void visit(IRule it) {
              MapSequence.fromMap(alternatives).put(it.getOutputConcept(), it);
            }
          });
  ...
  public SNode reparse(SNode rootNode, int timeout) {
    ...
    if (parseTree != null) {
      ...
      ListSequence.fromList(SNodeOperations.getNodeDescendants(newTree, CONCEPTS.ArbitraryTextAnnotation$hv, false, new SAbstractConcept[]{})).visitAll(new IVisitor<SNode>() {
  ...
  public PNode parse(SNode input, int timeout) {
    ...
    return parse(tokens, check_jpiiov_b0b0fb(input.getContainmentLink()), timeout);
  }
  ...
  public PNode parse(List<IToken> tokens, @Nullable SAbstractConcept expectedConcept, int timeout) {
    try {
      ...
      final Set<String> textTokens = SetSequence.fromSetWithValues(new HashSet<String>(), ListSequence.fromList(tokens).ofType(TextToken.class).select(new ISelector<TextToken, String>() {
        public String select(TextToken it) {
          return it.getTextValue();
        }
      }));

      myPrefilteredRules = ListSequence.fromList(rules).where(new IWhereFilter<IRule>() {
        public boolean accept(IRule it) {
          return ListSequence.fromList(it.getSymbols()).ofType(ConstantSymbol.class).all(new IWhereFilter<ConstantSymbol>() {
            public boolean accept(ConstantSymbol it) {
      ...
      final Set<SAbstractConcept> originalConcepts = SetSequence.fromSetWithValues(new HashSet<SAbstractConcept>(), ListSequence.fromList(tokens).select(new ISelector<IToken, SNode>() {
        public SNode select(IToken it) {
          return getOriginalNode(it);
        }
      ...
      myPrefilteredRules = ListSequence.fromList(myPrefilteredRules).sort(new ISelector<IRule, Integer>() {
        public Integer select(IRule it) {
          return (SetSequence.fromSet(originalConcepts).contains(it.getOutputConcept()) ? -1 : 0);
        }
      }, true).alsoSort(new ISelector<IRule, Integer>() {
        public Integer select(IRule it) {
      ...
      if (LOG_1855564834.isWarningLevel()) {
        LOG_1855564834.warning("Parsing failed", ex);
      }
      return null;
    } finally {
  ...
  public PNode parse(InputCursor input, @Nullable final SAbstractConcept expectedConcept, @Nullable SymbolInRule symbolInParent) {
    ...
    try {
      ...
      if (expectedConcept != null) {
        rules = ListSequence.fromList(rules).where(new IWhereFilter<IRule>() {
          public boolean accept(IRule it) {
            return SConceptOperations.isSubConceptOf(SNodeOperations.asSConcept(it.getOutputConcept()), SNodeOperations.asSConcept(expectedConcept));
          }
        }).toListSequence();
      }

      // prefer concept that the token had before destruction
      Set<SAbstractConcept> originalConcepts = SetSequence.fromSetWithValues(new HashSet<SAbstractConcept>(), ListSequence.fromList(input.getAll()).select(new ISelector<IToken, SAbstractConcept>() {
        public SAbstractConcept select(IToken it) {
          return MapSequence.fromMap(myOriginalConcept).get(it);
        }
      ...
      if (SetSequence.fromSet(originalConcepts).count() == 1) {
        ...
        rules = ListSequence.fromList(rules).where(new IWhereFilter<IRule>() {
          public boolean accept(IRule it) {
            return Objects.equals(it.getOutputConcept(), preferedConcept);
          }
        }).concat(ListSequence.fromList(rules).where(new IWhereFilter<IRule>() {
          public boolean accept(IRule it) {
            return !(Objects.equals(it.getOutputConcept(), preferedConcept));
          }
        })).toListSequence();
...
rangeLoop:
    while (rangesIterator.hasNext()) {
      ...
      if (Sequence.fromIterable(Sequence.fromArray(ranges)).any(new IWhereFilter<InputCursor>() {
        public boolean accept(InputCursor it) {
          return ListSequence.fromList(it.getAll()).count() == 0;
        }
      })) {
        continue;
      }
  ...
  protected boolean containsRequiredConstants(InputCursor input, List<ISymbol> symbols) {
    ...
    for (final ISymbol symbol : ListSequence.fromList(symbols)) {
      if (isConstant(symbol)) {
        if (ListSequence.fromList(tokens).all(new IWhereFilter<IToken>() {
          public boolean accept(IToken token) {
            return !(symbol.matches(token));
          }
        })) {
          return false;
        }
  ...
  protected List<IToken> getTokens(final SNode node) {
    ...
    ListSequence.fromList(result).visitAll(new IVisitor<IToken>() {
      public void visit(IToken it) {
  ...
  protected List<IToken> getArbitraryTextTokens(SNode node, final boolean left) {
    ...
    if (!(myIgnoreArbitraryText)) {
      for (SNode arbitraryText : ListSequence.fromList(new IAttributeDescriptor.NodeAttribute(CONCEPTS.ArbitraryTextAnnotation$hv).list(node)).where(new IWhereFilter<SNode>() {
        public boolean accept(SNode it) {
  ...
  protected SNode commonAncestor(List<SNode> nodes) {
    return ListSequence.fromList(nodes).foldLeft(((SNode) null), new ILeftCombinator<SNode, SNode>() {
      public SNode combine(SNode s, SNode it) {
        return commonAncestor(s, it);
      }
    });
  }
  ...
  protected void moveArbitraryTextToLeaf(SNode atext) {
    ...
    if (rule != null) {
      ...
      IToken firstLastToken = check_jpiiov_a0b0c0tc(firstLastSymbol, annotatedNode);
      if (check_jpiiov_a2a2a17(firstLastToken)) {
  ...
  public SNode isEndOf(SNode nodeToCheck, boolean leftEnd, SAbstractConcept ofConcept, @Nullable final SContainmentLink link) {
    ...
    if (link != null && SNodeOperations.isInstanceOf(parent, SNodeOperations.asSConcept(ofConcept))) {
      endSymbol = ListSequence.fromList(symbols).findFirst(new IWhereFilter<ISymbol>() {
        public boolean accept(ISymbol it) {
          return Objects.equals(it.getLink(), link);
        }
      });
    } else {
  ...
  public SNode findRootExpression(SNode start) {
    return ListSequence.fromList(SNodeOperations.getNodeAncestors(start, null, true)).findFirst(new IWhereFilter<SNode>() {
      public boolean accept(SNode it) {
        return (SNodeOperations.getParent(it) == null) || getRule(SNodeOperations.getConcept(SNodeOperations.getParent(it))) == null;
      }
    });
  }

  private static SAbstractConcept check_jpiiov_b0b0fb(SContainmentLink checkedDotOperand) {
    if (null != checkedDotOperand) {
      return checkedDotOperand.getTargetConcept();
    }
    return null;
  }
  private static IToken check_jpiiov_a0b0c0tc(ISymbol checkedDotOperand, SNode annotatedNode) {
    if (null != checkedDotOperand) {
      return checkedDotOperand.read(annotatedNode);
    }
    return null;
  }
  private static boolean check_jpiiov_a2a2a17(IToken checkedDotOperand) {
    if (null != checkedDotOperand) {
