...
public class LayoutableCollectionAdapter extends AbstractLayoutableAdapter implements ILayoutableContainer {
  private static final Logger LOG_123413612 = Logger.getLogger(LayoutableCollectionAdapter.class);
  private static final org.apache.log4j.Logger LOG = org.apache.log4j.Logger.getLogger(LayoutableCollectionAdapter.class);
  ...
  public static ILayouter getLayouter(EditorCell_Collection cell) {
    ...
    if (Objects.equals(cellLayout.getClass(), CellLayout_Vertical.class)) {
      if (((Boolean) ReflectionUtil_copy.callMethod(CellLayout_Vertical.class, ((CellLayout_Vertical) cellLayout), "isGridLayout", new Class[]{}, new Object[]{}))) {
    ...
    if (LOG_123413612.isDebugLevel()) {
      LOG_123413612.debug("Unsupported layout: " + cellLayout.getClass().getName());
    }
    return null;
  }
  ...
  public boolean calcHasBorder(boolean left) {
    ...
    if (ListSequence.fromList(getChildren()).count() > 0) {
      ...
      if (check_z1h9za_a1a1a49(as_z1h9za_a0a1a1a59(child, AbstractLayoutableAdapter.class), left)) {
        return true;
      }
    }
    return false;
  ...
  protected boolean calcIsPushX() {
    return (myCell.getStyle().isSpecified(PUSH_X) ? myCell.getStyle().get(PUSH_X) : myCell.getStyle().get(PUSH_X) || ListSequence.fromList(getChildren()).any(new IWhereFilter<ILayoutable>() {
      public boolean accept(ILayoutable it) {
        return it.isPushX();
      }
    }));
  }
  ...
  protected boolean calcIsPushY() {
    return (myCell.getStyle().isSpecified(PUSH_Y) ? myCell.getStyle().get(PUSH_Y) : myCell.getStyle().get(PUSH_Y) || ListSequence.fromList(getChildren()).any(new IWhereFilter<ILayoutable>() {
      public boolean accept(ILayoutable it) {
        return it.isPushY();
      }
    }));
  ...
  public ILayoutable getBaselineChild() {
    ...
    switch (bL) {
      ...
      case CENTER:
        ...
        return LayoutableAdapters.getAdapter(ListSequence.fromList(Sequence.fromIterable(children).toListSequence()).getElement(Sequence.fromIterable(children).count() / 2));
    }
  ...
  private static boolean check_z1h9za_a1a1a49(AbstractLayoutableAdapter checkedDotOperand, Boolean left) {
    if (null != checkedDotOperand) {
      return checkedDotOperand.hasBorder(left);
    }
    return false;
  }
  private static <T> T as_z1h9za_a0a1a1a59(Object o, Class<T> type) {
