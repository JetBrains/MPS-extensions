...
public class TableEditor extends AbstractTableEditor {
  ...
  private void addHeadersToThis(Grid grid) {
    for (Header header : SetSequence.fromSet(grid.getAllHeaders()).where(new IWhereFilter<Header>() {
      public boolean accept(Header it) {
        return it instanceof EditorCellHeader;
      }
    })) {
  ...
  public void paintCell(Graphics g, ParentSettings parentSettings) {
    ...
    for (IGridElement element : getGrid()) {
      EditorCellGridLeaf leaf = as_cj6dzx_a0a0a9a53(element, EditorCellGridLeaf.class);
      EditorCell child = check_cj6dzx_a0b0j0jb(leaf);
      if (child == null) {
        continue;
      }
  ...
  private void wrapGridCells(Grid grid) {
    ...
    for (int x = 0; x < grid.getSizeX(); x++) {
      for (int y = 0; y < grid.getSizeY(); y++) {
        EditorCellGridLeaf leaf = as_cj6dzx_a0a0a0a1a73(grid.getElement(x, y), EditorCellGridLeaf.class);
        if (leaf == null) {
          continue;
        }

        EditorCell_GridCell wrapper = as_cj6dzx_a0a3a0a1a73(leaf.getEditorCell(), EditorCell_GridCell.class);
        if (wrapper == null) {
  ...
  private static void setAllIndentLayoutMaxWidth(EditorCell parent, Integer width) {
    if (as_cj6dzx_a0a0a14(parent, EditorCell_Collection.class) != null) {
      Style style = parent.getStyle();
      if (!(style.isSpecified(StyleAttributes.MAX_WIDTH))) {
        style.set(StyleAttributes.MAX_WIDTH, width);
      }
    }
    for (EditorCell child : Sequence.fromIterable(as_cj6dzx_a0a1a14(parent, jetbrains.mps.openapi.editor.cells.EditorCell_Collection.class))) {
  ...
  private static EditorCell check_cj6dzx_a0b0j0jb(EditorCellGridLeaf checkedDotOperand) {
    if (null != checkedDotOperand) {
      return checkedDotOperand.getEditorCell();
    }
    return null;
  }
  private static <T> T as_cj6dzx_a0a0a9a53(Object o, Class<T> type) {
    return (type.isInstance(o) ? (T) o : null);
  }
  private static <T> T as_cj6dzx_a0a0a0a1a73(Object o, Class<T> type) {
    return (type.isInstance(o) ? (T) o : null);
  }
  private static <T> T as_cj6dzx_a0a3a0a1a73(Object o, Class<T> type) {
    return (type.isInstance(o) ? (T) o : null);
  }
  private static <T> T as_cj6dzx_a0a0a14(Object o, Class<T> type) {
    return (type.isInstance(o) ? (T) o : null);
  }
  private static <T> T as_cj6dzx_a0a1a14(Object o, Class<T> type) {
    return (type.isInstance(o) ? (T) o : null);
  }
}
