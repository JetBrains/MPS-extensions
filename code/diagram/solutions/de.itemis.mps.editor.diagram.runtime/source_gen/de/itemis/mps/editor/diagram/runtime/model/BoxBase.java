...
public abstract class BoxBase extends DiagramElement {
  ...
  public boolean isScalingAllowed() {
    return myScalingAllowed && ListSequence.fromList(myContentCells).select(new ISelector<EditorCell, Boolean>() {
      public Boolean select(EditorCell it) {
        return ScalingInfoUtil.allowScaling(it);
      }
    }).reduceLeft(new ILeftCombinator<Boolean, Boolean>() {
