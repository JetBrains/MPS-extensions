...
public class ProjectViewManager implements DeployListener {
  ...
  public void onUnloaded(@NotNull Set<ReloadableModule> unloadedModules, @NotNull ProgressMonitor monitor) {
    for (ReloadableModule module : SetSequence.fromSet(unloadedModules)) {
      ...
      if (Objects.equals(module.getModuleReference(), PersistenceFacade.getInstance().createModuleReference("732373f1-0d48-4122-bb43-c1606db4baca(com.mbeddr.mpsutil.projectview.runtime)"))) {
        ThreadUtils.runInUIThreadNoWait(() -> dispose());
        dispose();
        return;
      }
  ...
  public void syncViews() {
    SetSequence.fromSet(myProjects).visitAll(new IVisitor<Project>() {
      public void visit(Project it) {
        syncViews(it);
      }
    });
  }
  ...
  protected void syncViews(final Project project) {
    ...
    if (SetSequence.fromSet(myProjects).contains(project)) {
      registeredViews.value = SetSequence.fromSetWithValues(new HashSet<ViewId>(), Sequence.fromIterable(ViewRegistry.getInstance().getVisibleDescriptors()).select(new ISelector<IViewDescriptor, ViewId>() {
        public ViewId select(IViewDescriptor it) {
          return it.getId();
        }
      }));
    }
    ...
    ThreadUtils.runInUIThreadNoWait(() -> {
      for (ViewId activeView : SetSequence.fromSet(MapSequence.fromMap(activeViews.value).keySet()).toListSequence()) {
  ...
  public void activateView(ViewId viewId, Project project) {
    ...
    Element paneState = ((Map<String, Element>) ReflectionUtil_copy.readField(ProjectViewImpl.class, ((ProjectViewImpl) projectView), "myUninitializedPaneState")).get(pane.getId());
    if (paneState != null) {
  ...
  public void install() {
    ...
    clsLoaderManager.addListener(this);
    final MPSModuleRepository moduleRepository = platform.findComponent(MPSModuleRepository.class);
  ...
  public void dispose() {
    ...
    clsLoaderManager.removeListener(this);

    ThreadUtils.runInUIThreadNoWait(() -> {
      for (IMapping<Project, Map<ViewId, CustomProjectView>> projectInstances : MapSequence.fromMap(myViewInstances).toListSequence()) {
        for (IMapping<ViewId, CustomProjectView> instance : MapSequence.fromMap(projectInstances.value()).toListSequence()) {
  ...
  public void loadDescriptor(IProjectViewDescriptor descriptor) {
    Sequence.fromIterable(descriptor.getViewDescriptors()).visitAll(new IVisitor<IViewDescriptor>() {
      public void visit(IViewDescriptor it) {
        ViewRegistry.getInstance().register(it);
      }
    });
  }
  ...
  public void unloadDescriptor(IProjectViewDescriptor descriptor) {
    Sequence.fromIterable(descriptor.getViewDescriptors()).visitAll(new IVisitor<IViewDescriptor>() {
      public void visit(IViewDescriptor it) {
        ViewRegistry.getInstance().unregister(it);
      }
    });
  }
}
