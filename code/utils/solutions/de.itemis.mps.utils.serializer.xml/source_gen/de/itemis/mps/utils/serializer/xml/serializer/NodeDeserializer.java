...
public class NodeDeserializer extends XMLConstants {
  ...
  private void resolveReferences(SNode newRootNode) {
    Iterable<SNode> proxies = ListSequence.fromList(SNodeOperations.getNodeDescendants(newRootNode, CONCEPTS.BaseConcept$gP, false, new SAbstractConcept[]{})).where(new IWhereFilter<SNode>() {
      public boolean accept(SNode it) {
        return new SNodeLegacy(it).getProperty("proxy") != null;
      }
    });
  ...
  private SNode resolveInExistingModel(final String modelID, final String nodeID) {
    ...
    return ListSequence.fromList(allNodes).findFirst(new IWhereFilter<SNode>() {
      public boolean accept(SNode it) {
        return it.getNodeId().toString().equals(nodeID) && SNodeOperations.getModel(it).getModelId().toString().equals(modelID);
      }
    });
  }
  ...
  private SNode resolveInNewSubtree(String originalNodeID, SNode newRoot) {
    ...
    return ListSequence.fromList(SNodeOperations.getNodeDescendants(newRoot, CONCEPTS.BaseConcept$gP, true, new SAbstractConcept[]{})).findFirst(new IWhereFilter<SNode>() {
      public boolean accept(SNode it) {
        return it.getNodeId().toString().equals(newID);
      }
    });
  }
