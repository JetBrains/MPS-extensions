...
/*package*/ class PartialTableWithRowHeaderQuery_Table_EditorBuilder_a extends AbstractEditorBuilder {
  @NotNull
  private SNode myNode;

  public PartialTableWithRowHeaderQuery_Table_EditorBuilder_a(@NotNull EditorContext context, @NotNull SNode node) {
    super(context);
    myNode = node;
  }
    ...
    _FunctionTypes._void_P0_E0 creator = () -> {
      CellCaching.maybeDisableCache(node, editorContext, () -> {
        try {
          ...
          final Grid childGrid = createChildsVertical_h3nbg4_a0(editorContext, node);
          ...
          editorCell.value.setCellId("Table_h3nbg4_a");
  ...
  public Grid createChildsVertical_h3nbg4_a0(final EditorContext editorContext, final SNode node) {
    Grid grid = new Grid();
    GridAdapter gridAdapter = new GridAdapter(grid, editorContext, node);


    final IHeaderNodeInsertAction insertAction = new ChildNodesInsertAction(node, SLinkOperations.findLinkDeclaration(LINKS.rows$O1FF)) {};
    ...
    try {
      ...
      int y = 0;
      Iterable<SNode> elements = SLinkOperations.getChildren(node, LINKS.rows$O1FF);
      for (SNode child : Sequence.fromIterable(elements)) {
        ...
        SubstituteInfo substituteInfo = SubstituteUtil.forChild(editorContext, node, (y < ListSequence.fromList(SLinkOperations.getChildren(node, LINKS.rows$O1FF)).count() ? ListSequence.fromList(SLinkOperations.getChildren(node, LINKS.rows$O1FF)).getElement(y) : null), LINKS.rows$O1FF);
    ...
    if (ListSequence.fromList(SLinkOperations.getChildren(node, LINKS.rows$O1FF)).isEmpty()) {
      ...
      SubstituteInfo substituteInfo = new DefaultSChildSubstituteInfo(node, null, LINKS.rows$O1FF, editorContext);
