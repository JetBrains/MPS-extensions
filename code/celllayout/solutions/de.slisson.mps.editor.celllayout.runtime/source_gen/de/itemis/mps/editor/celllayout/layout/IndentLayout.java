...
public class IndentLayout extends AbstractLayout implements ILayouter {
  ...
  public void layoutIntermediateCollections(final ILayoutableContainer container) {
    List<ILayoutableContainer> list = Sequence.fromIterable(getContainers(container)).where(new IWhereFilter<ILayoutableContainer>() {
      public boolean accept(ILayoutableContainer it) {
        return it != container && isIntermediateCollection(it);
      }
    }).toListSequence();
    ListSequence.fromList(list).visitAll(new IVisitor<ILayoutableContainer>() {
      public void visit(ILayoutableContainer it) {
  ...
  protected Iterable<ILayoutable> getIndentLeafs(ILayoutableContainer container) {
    return Sequence.fromIterable(loadWords(container, (ILayoutable l, Integer indentSize) -> Size.ZERO)).select(new ISelector<Word, ILayoutable>() {
      public ILayoutable select(Word it) {
  ...
  public Size getMinInnerSize(ILayoutableContainer container, @NotNull final Size sizeConstraint) {
    ...
    int maxWidth = Sequence.fromIterable(words).select(new ISelector<Word, Integer>() {
      public Integer select(Word it) {
        return SizeExtensions.zeroIfOverflow(it.getLayoutable().getMinSize(sizeConstraint), it.getLayoutable()).getWidth();
      }
  ...
  public int getAscent(ILayoutableContainer container, final Size sizeConstraint) {
    ...
    if (baselineChild != null) {
      ...
      for (Line line : Sequence.fromIterable(text.getLines())) {
        if (ListSequence.fromList(line.getWords()).any(new IWhereFilter<Word>() {
          public boolean accept(Word it) {
  ...
  protected Iterable<Word> loadWords(ILayoutable container, final int treeDepth, int indentLevel, int parentMaxWidth, final _FunctionTypes._return_P2_E0<? extends Size, ? super ILayoutable, ? super Integer> sizeGetter) {
    ...
    if (isIntermediateCollection(container) || treeDepth == 0) {
      return ListSequence.fromList(((ILayoutableContainer) container).getChildren()).translate(new ITranslator2<ILayoutable, Word>() {
        public Iterable<Word> translate(ILayoutable it) {
          return loadWords(it, treeDepth + 1, _indentLevel.value, maxWidth, sizeGetter);
        }
      });
    } else {
  ...
  protected Iterable<ILayoutableContainer> getContainers(ILayoutable container) {
    if (container instanceof ILayoutableContainer && isIndentLayout((ILayoutableContainer) container)) {
      return ListSequence.fromList(((ILayoutableContainer) container).getChildren()).translate(new ITranslator2<ILayoutable, ILayoutableContainer>() {
        public Iterable<ILayoutableContainer> translate(ILayoutable it) {
          return getContainers(it);
        }
      }).concat(Sequence.fromIterable(Sequence.<ILayoutableContainer>singleton(((ILayoutableContainer) container))));
    } else {
  ...
  public class Text {
    ...
    public String toString() {
      return IterableUtils.join(ListSequence.fromList(myLines).select(new ISelector<Line, String>() {
        public String select(Line it) {
          return it.toString();
        }
      }), "\n");
    }
    ...
    public int getHeight() {
      return ListSequence.fromList(myLines).foldLeft(0, new ILeftCombinator<Line, Integer>() {
        public Integer combine(Integer s, Line it) {
          return s + it.getHeight();
        }
      });
    }

    public int getWidth() {
      int width = ListSequence.fromList(myLines).foldLeft(0, new ILeftCombinator<Line, Integer>() {
        public Integer combine(Integer s, Line it) {
          return Math.max(s, it.getWidth());
        }
      });
      return width;
    }
  ...
  public class Line {
    ...
    @Override
    public String toString() {
      return IterableUtils.join(ListSequence.fromList(myWords).select(new ISelector<Word, String>() {
        public String select(Word it) {
          return it.toString();
        }
      }), " ");
    }
    ...
    private int getDescent() {
      return ListSequence.fromList(myWords).select(new ISelector<Word, Integer>() {
        public Integer select(Word it) {
          return it.getDescent();
        }
      }).foldLeft(0, new ILeftCombinator<Integer, Integer>() {
        public Integer combine(Integer s, Integer it) {
          return Math.max(s, it);
    ...
    public void applyLayout(final int textX, final int textY, int targetWidth) {
      ...
      if (growAmount > 0) {
        List<Word> growingWords = ListSequence.fromList(getWords()).where(new IWhereFilter<Word>() {
          public boolean accept(Word it) {
            return isChildGrowX(it.getLayoutable());
          }
        }).toListSequence();
