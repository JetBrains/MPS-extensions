...
public class GraphSelection extends AbstractSelection implements SingularSelection {
  ...
  private void moveBy(final double dx, final double dy) {
    ...
    Object[] cellsToMove = ListSequence.fromList(mySelectedCells).select(new ISelector<mxCell, Cloneable>() {
      public Cloneable select(mxCell it) {
...

  {
    MapSequence.fromMap(myActions).put(CellActionType.DELETE, () -> {
      if (DiagramModel.ALLOW_DELETE_DURING_SYNC) {
        ...
        myDiagramCell.getEditor().getEditorContext().getRepository().getModelAccess().executeCommand(() -> {
          final Set<IDiagramElement> elementsToDelete = SetSequence.fromSetWithValues(new HashSet<IDiagramElement>(), Sequence.fromIterable(Sequence.fromArray(getDiagramCell().getGraph().getSelectionCells())).ofType(IDiagramDCell.class).select(new ISelector<IDiagramDCell, IDiagramElement>() {
            public IDiagramElement select(IDiagramDCell it) {
              return it.getDiagramElement();
            }
          }));
          SetSequence.fromSet(elementsToDelete).addSequence(Sequence.fromIterable(getDiagramCell().getModel().getEdges()).where(new IWhereFilter<Edge>() {
          ...
          SetSequence.fromSet(elementsToDelete).visitAll(new IVisitor<IDiagramElement>() {
            public void visit(IDiagramElement it) {
              it.delete();
            }
          });
        });
    ...
    MapSequence.fromMap(myActions).put(CellActionType.LEFT, () -> moveBy(-MyGraphComponent.ARROW_KEY_MOVE_AMOUNT, 0));
    MapSequence.fromMap(myActions).put(CellActionType.RIGHT, () -> moveBy(MyGraphComponent.ARROW_KEY_MOVE_AMOUNT, 0));
    MapSequence.fromMap(myActions).put(CellActionType.UP, () -> moveBy(0, -MyGraphComponent.ARROW_KEY_MOVE_AMOUNT));
    MapSequence.fromMap(myActions).put(CellActionType.DOWN, () -> moveBy(0, MyGraphComponent.ARROW_KEY_MOVE_AMOUNT));

    MapSequence.fromMap(myActions).put(CellActionType.SELECT_LEFT, () -> moveBy(-MyGraphComponent.ARROW_KEY_MOVE_AMOUNT_SLOW, 0));
    MapSequence.fromMap(myActions).put(CellActionType.SELECT_RIGHT, () -> moveBy(MyGraphComponent.ARROW_KEY_MOVE_AMOUNT_SLOW, 0));
    MapSequence.fromMap(myActions).put(CellActionType.SELECT_PREVIOUS, () -> moveBy(0, -MyGraphComponent.ARROW_KEY_MOVE_AMOUNT_SLOW));
    MapSequence.fromMap(myActions).put(CellActionType.SELECT_NEXT, () -> moveBy(0, MyGraphComponent.ARROW_KEY_MOVE_AMOUNT_SLOW));
  }
  ...
  public List<EditorCell> getSelectedCells() {
    if (ListSequence.fromList(mySelectedCells).count() > 0) {
      final Wrappers._T<List<EditorCell>> cells = new Wrappers._T<List<EditorCell>>(ListSequence.fromList(mySelectedCells).select(new ISelector<mxCell, EditorCell>() {
        public EditorCell select(mxCell it) {
          return extractEditorCell(it);
        }
      ...
      if (ListSequence.fromList(cells.value).any(new IWhereFilter<EditorCell>() {
        public boolean accept(EditorCell it) {
          SNode node = it.getSNode();
          return SNodeOperations.isInstanceOf(node, CONCEPTS.Attribute$g1);
        }
      })) {
        myDiagramCell.getEditorComponent().getEditorContext().getRepository().getModelAccess().runReadAction(new _Adapters._return_P0_E0_to_Runnable_adapter(new _FunctionTypes._return_P0_E0<List<EditorCell>>() {
          public List<EditorCell> invoke() {
            return cells.value = ListSequence.fromList(cells.value).select(new ISelector<EditorCell, EditorCell>() {
              public EditorCell select(EditorCell cell) {
  ...
  public List<SNode> getSelectedNodes() {
    ...
    return ListSequence.fromList(selectedCells).select(new ISelector<EditorCell, SNode>() {
      public SNode select(EditorCell it) {
        return it.getSNode();
      }
  ...
  public EditorCell findCell(CellInfo cellInfo, EditorComponent editorComponent) {
    if (cellInfo instanceof DefaultCellInfo) {
      ...
      SNode node = ((SNodeReference) ReflectionUtil_copy.readField(DefaultCellInfo.class, defaultInfo, "myNodeReference")).resolve(editorComponent.getEditorContext().getRepository());
      return findCell(node, ((String) ReflectionUtil_copy.readField(DefaultCellInfo.class, defaultInfo, "myCellId")), editorComponent);
    }
  ...
  public EditorCell findInVisibleCells(@NotNull final String cellId, @Nullable final SNode snode) {
    ...
    EditorCell found = SetSequence.fromSet(MapSequence.fromMap(map).keySet()).where(new IWhereFilter<EditorCell>() {
      public boolean accept(EditorCell it) {
        return it != null;
      }
    }).findFirst(new IWhereFilter<EditorCell>() {
      public boolean accept(EditorCell it) {
        return Objects.equals(it.getCellId(), cellId) && (snode == null || Objects.equals(snode, it.getSNode()));
      }
    });
    if (found != null) {
      return found;
