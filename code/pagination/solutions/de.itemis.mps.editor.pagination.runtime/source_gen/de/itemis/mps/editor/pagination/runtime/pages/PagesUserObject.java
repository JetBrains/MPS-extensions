...
public class PagesUserObject {
  ...
  public static Pages getOrCreate(SNode nodeWithPagination, final SContainmentLink linkToPaginate, int pageSize) {
    ...
    if (pages == null || pages.getCurrentPage().getPageSize() != pageSize) {
      pages = new Pages(ListSequence.fromList(SNodeOperations.getChildren(nodeWithPagination)).where(new IWhereFilter<SNode>() {
        public boolean accept(SNode c) {
          return Objects.equals(SNodeOperations.getContainingLink(c), linkToPaginate);
        }
      }), pageSize);
