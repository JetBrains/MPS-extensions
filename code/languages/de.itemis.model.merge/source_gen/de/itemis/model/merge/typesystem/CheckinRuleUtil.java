...
public class CheckinRuleUtil {
  ...
  public static Set<SNode> conceptsWithMissingMergePolicy(SNode modelMerge) {
    try {
      MergeResolverFromModelMerge.run(modelMerge);
    } catch (MissingMergePolicies mp) {
  ...
  public static <X> void warn(Iterable<SAbstractConcept> concepts, SRepository repo, Map<SAbstractConcept, Set<X>> conceptToX, final _FunctionTypes._return_P1_E0<? extends Set<X>, ? super SNode> needToCoverXs, final _FunctionTypes._void_P2_E0<? super SAbstractConcept, ? super X> warning) {
    for (final SAbstractConcept c : Sequence.fromIterable(concepts)) {
      ...
      Iterable<SNode> allSuperConcepts = Sequence.fromIterable(AbstractConceptDeclaration__BehaviorDescriptor.getAllSuperConcepts_id2A8AB0rAWpG.invoke(declarationNode, ((boolean) true))).where(new IWhereFilter<SNode>() {
        public boolean accept(SNode it) {
          return !((boolean) AbstractConceptDeclaration__BehaviorDescriptor.is_id4MKjpUYmGW0.invoke(it, CONCEPTS.BaseConcept$gP));
        }
      });
      HashSet<X> allX = Sets.newHashSet(Sequence.fromIterable(allSuperConcepts).translate(new ITranslator2<SNode, X>() {
        public Iterable<X> translate(SNode it) {
          return needToCoverXs.invoke(it);
        }
      }));
      Set<X> xsWithoutMergePolicies = Sets.difference(allX, existentX);
      SetSequence.fromSet(xsWithoutMergePolicies).visitAll(new IVisitor<X>() {
        public void visit(X it) {
          warning.invoke(c, it);
        }
      });
    }
  ...
  public static SNode warningNode(SNode modelMerge, final SAbstractConcept sac) {
    SNode warningNode = Sequence.fromIterable(SNodeOperations.ofConcept(SLinkOperations.getChildren(modelMerge, LINKS.items$z8dk), CONCEPTS.MergePolicy$RB)).findFirst(new IWhereFilter<SNode>() {
      public boolean accept(SNode it) {
        return (boolean) AbstractConceptDeclaration__BehaviorDescriptor.is_id4MKjpUYmGW0.invoke(SLinkOperations.getTarget(it, LINKS.conceptRef$p8vY), sac);
      }
    });
