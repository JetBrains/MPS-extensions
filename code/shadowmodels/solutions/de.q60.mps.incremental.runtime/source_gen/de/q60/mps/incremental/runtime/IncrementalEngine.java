...
public class IncrementalEngine {
  ...
  protected synchronized void doInvalidate(Iterable<DependencyKey> changes) {
    ...
    List<DependencyKey> cacheValueEvents = SetSequence.fromSet(invalidatedKeys).select(new ISelector<Object, DependencyKey>() {
      public DependencyKey select(Object it) {
    ...
    SetSequence.fromSet(watchedEntries).where(new IWhereFilter<WatchedEntry>() {
      public boolean accept(WatchedEntry it) {
        return SetSequence.fromSet(invalidatedKeys).contains(it.incrementalComputable.key);
      }
    }).visitAll(new IVisitor<WatchedEntry>() {
      public void visit(WatchedEntry it) {
        it.notifyListener();
      }
    });
  }
