...
public class AccessorFactory implements IAccessorFactory {
  ...
  public IEdgeAccessor edgeFromSNode(@NotNull SNode node) {
    ...
    return as_q86kb5_a0b0k(ListSequence.fromList(fromSNode).where(new IWhereFilter<IDiagramElementAccessor>() {
      public boolean accept(IDiagramElementAccessor it) {
        return it instanceof IEdgeAccessor;
      }
    }).first(), IEdgeAccessor.class);
  }
  @Nullable
  @Override
  public IBoxAccessor boxFromSNode(@NotNull SNode node) {
    List<IDiagramElementAccessor> fromSNode = fromSNode(node, false);
    return as_q86kb5_a0b0l(ListSequence.fromList(fromSNode).where(new IWhereFilter<IDiagramElementAccessor>() {
      public boolean accept(IDiagramElementAccessor it) {
        return it instanceof IBoxAccessor;
      }
    }).first(), IBoxAccessor.class);
  }
