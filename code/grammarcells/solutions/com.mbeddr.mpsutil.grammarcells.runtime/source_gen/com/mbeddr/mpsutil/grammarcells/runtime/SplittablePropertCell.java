...
public class SplittablePropertCell extends EditorCell_Property {
  ...
  protected void processSplit() {
    ...
    getEditorComponent().getEditorContext().getRepository().getModelAccess().runReadAction(() -> {
      final List<String> propertyStringTokens = ListSequence.fromList(myTokenizer.tokenize(getTextBeforeCaret(), getTextAfterCaret())).where(new IWhereFilter<String>() {
      ...
      nodeTokens = ListSequence.fromList(nodeTokens).translate(new ITranslator2<IToken, IToken>() {
        public Iterable<IToken> translate(IToken it) {
          if (it instanceof PropertyToken && Objects.equals(((PropertyToken) it).getProperty(), ((PropertyAccessor) getModelAccessor()).getProperty())) {
            return ListSequence.fromList(propertyStringTokens).select(new ISelector<String, IToken>() {
              public IToken select(String it) {
