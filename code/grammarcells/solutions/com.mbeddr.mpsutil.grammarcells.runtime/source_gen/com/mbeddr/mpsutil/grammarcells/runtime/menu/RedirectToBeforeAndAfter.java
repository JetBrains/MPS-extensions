  ...
  protected static List<TransformationMenuItem> getBeforeAfterActions(final EditorCell anchorCell, final boolean before, final TransformationMenuContext context) {
    ...
    if (anchorCell.getSNode().isInstanceOfConcept(CONCEPTS.Attribute$g1)) {
      ...
      if (attributedCell != null) {
        alignedCells = ListSequence.fromList(alignedCells).concat(ListSequence.fromList(getAlignedCells(attributedCell, before))).distinct().toListSequence();
      }
    }

    List<TransformationMenuItem> result = ListSequence.fromList(alignedCells).translate(new ITranslator2<EditorCell, TransformationMenuItem>() {
      public Iterable<TransformationMenuItem> translate(EditorCell cell) {
        return RedirectToBeforeAndAfter.createActionsForCell(cell, beforeAfterContext);
      }
    }).toListSequence();

    // include actions from empty collections
    List<EditorCell_Collection> emptyCollections = ListSequence.fromList(alignedCells).select(new ISelector<EditorCell, EditorCell>() {
      public EditorCell select(EditorCell it) {
        return getPreviousNextSibling(before, it, (EditorCell c) -> GrammarCellsUtil.isSyntaxCell(c));
      }
    }).ofType(EditorCell_Collection.class).where(new IWhereFilter<EditorCell_Collection>() {
      public boolean accept(EditorCell_Collection it) {
        return it.getCellsCount() == 0;
      }
    }).toListSequence();
    result = ListSequence.fromList(result).concat(ListSequence.fromList(emptyCollections).translate(new ITranslator2<EditorCell_Collection, TransformationMenuItem>() {
      public Iterable<TransformationMenuItem> translate(EditorCell_Collection it) {
        return Sequence.fromIterable(createActionsForCell(it, context.withLocation(GrammarCellsMenuLocations.BEFORE))).concat(Sequence.fromIterable(createActionsForCell(it, context.withLocation(GrammarCellsMenuLocations.AFTER))));
      }
    })).toListSequence();
    ...
    public class Menu implements TransformationMenu {
      ...
      public List<TransformationMenuItem> createMenuItems(@NotNull TransformationMenuContext context) {
        ...
        try {
          ...
          if (stHint != null) {
            anchorCell = ((EditorCell) ReflectionUtil_copy.readField(EditorCell_STHint.class, stHint, "myAnchorCell"));
          }
