...
public class SubgraphStage implements ISubgraphStage {
  ...
  public SubgraphStage(ITransformationEngine engine, SubstepStageReference stageRef, SubstepStageReference previousStageRef, Iterable<IContainmentTarget> roots) {
    ...
    this.roots = Sequence.fromIterable(roots).select(new ISelector<IContainmentTarget, IUniqueContainmentTarget>() {
      public IUniqueContainmentTarget select(IContainmentTarget it) {
  ...
  public Iterable<IUniqueContainmentTarget> resolveContent(IContainmentTarget target) {
    ...
    return Sequence.fromIterable(targets).where(new IWhereFilter<IUniqueContainmentTarget>() {
  ...
  public Iterable<IOutputNodeReference> resolveContainmentTarget(IUniqueContainmentTarget target) {
    if (target.getTarget() instanceof TransformationCall) {
      ...
      return Sequence.fromIterable(tresult.getRootNodes()).select(new ISelector<IOutputNodeReference, IOutputNodeReference>() {
        public IOutputNodeReference select(IOutputNodeReference it) {
          return applyWeavings(it);
        }
      });
      ...
      return Sequence.fromIterable(Sequence.<IOutputNodeReference>singleton(new CopiedOutputNodeReference(getStageReference(), (IUniqueContainmentTarget<CopyCall>) target))).select(new ISelector<IOutputNodeReference, IOutputNodeReference>() {
        public IOutputNodeReference select(IOutputNodeReference it) {
          return applyWeavings(it);
        }
      });
  ...
  public Iterable<MappingLabels> getMappingLabels() {
    checkFrozen();
    return SetSequence.fromSet(callsWithLabels).where(new IWhereFilter<IUniqueContainmentTarget>() {
      public boolean accept(IUniqueContainmentTarget it) {
        return it.getTarget() instanceof TransformationCall;
      }
    }).select(new ISelector<IUniqueContainmentTarget, ITransformationResult>() {
      public ITransformationResult select(IUniqueContainmentTarget it) {
        return getTransformationResult(it);
      }
    }).select(new ISelector<ITransformationResult, MappingLabels>() {
      public MappingLabels select(ITransformationResult it) {
        return it.mappingLabels();
      }
    });
  }
  ...
  public Iterable<ScopeImport> getDeclaredImports() {
    return SetSequence.fromSet(callsWithDependencies).select(new ISelector<IUniqueContainmentTarget, ITransformationResult>() {
      public ITransformationResult select(IUniqueContainmentTarget it) {
        return getTransformationResult(it);
      }
    }).translate(new ITranslator2<ITransformationResult, ScopeImport>() {
      public Iterable<ScopeImport> translate(ITransformationResult it) {
