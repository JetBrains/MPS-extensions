...
public class SM_ShadowOutputSynchronizerStep2 {
  ...
  public SNode createNode(final INode input) {
    ...
    if (SConceptOperations.isSubConceptOf(SNodeOperations.asSConcept(concept), CONCEPTS.INamedConcept$Kd)) {
      candidates = Sequence.fromIterable(candidates).where(new IWhereFilter<SNode>() {
        public boolean accept(SNode it) {
          return Objects.equals(SPropertyOperations.getString(SNodeOperations.cast(it, CONCEPTS.INamedConcept$Kd), PROPS.name$MnvL), SPropertyOperations.getString(SNodeOperations.cast(NodeToSNodeAdapter.wrap(input), CONCEPTS.INamedConcept$Kd), PROPS.name$MnvL));
        }
      });
    }
  ...
  public SModule getOrCreateModule(final INode input) {
    ...
    SModule module = SetSequence.fromSet(createdModules).findFirst(new IWhereFilter<SM_ShadowModule>() {
      public boolean accept(SM_ShadowModule it) {
        return Objects.equals(it.getModuleName(), name.value) && Objects.equals(it.getUserObject(INPUT_NODE_UOKEY), input);
      }
    });
    if (module == null) {
      Iterable<SModule> modules = repository.getModules();
      final String id = input.getPropertyValue(PROPS.id$7MjP.getName());
      module = Sequence.fromIterable(modules).findFirst(new IWhereFilter<SModule>() {
        public boolean accept(SModule it) {
          return Objects.equals(it.getModuleId().toString(), id);
        }
      });
    }
    if (module == null) {
  ...
  public SModel getOrCreateModel(SModule module, final INode input) {
    ...
    SModel model = Sequence.fromIterable(models).ofType(SModel.class).findFirst(new IWhereFilter<SModel>() {
    ...
    if (model == null) {
      if (module instanceof SM_ShadowModule) {
        ...
        ModelRoot modelRoot = Sequence.fromIterable(modelRoots).where(new IWhereFilter<ModelRoot>() {
          public boolean accept(ModelRoot it) {
            return it.canCreateModel(name.value);
          }
        }).first();
        if (modelRoot == null) {
          if (LOG.isErrorLevel()) {
  ...
  public void synchronizeRepository(SNode repositorySpec, SRepository repository) {
    ...
    List<SM_ShadowModule> expectedModules = ListSequence.fromList(SLinkOperations.getChildren(repositorySpec, LINKS.modules$jBPn)).select(new ISelector<SNode, SModule>() {
      public SModule select(SNode it) {
        return getOrCreateModule(SNodeToNodeAdapter.wrap(it));
      }
    }).ofType(SM_ShadowModule.class).toListSequence();
    List<SM_ShadowModule> modulesToRemove = SetSequence.fromSet(createdModules).subtract(ListSequence.fromList(expectedModules)).toListSequence();
    List<SM_ShadowModule> modulesToAdd = ListSequence.fromList(expectedModules).subtract(SetSequence.fromSet(createdModules)).toListSequence();
    ...
    ListSequence.fromList(modulesToRemove).visitAll(new IVisitor<SM_ShadowModule>() {
      public void visit(SM_ShadowModule it) {
        removeModule(it);
      }
    });
  }
  ...
  private void checkDuplicateModuleIds(SNode repositorySpec) {
    ...
    for (String moduleId : ListSequence.fromList(SLinkOperations.getChildren(repositorySpec, LINKS.modules$jBPn)).select(new ISelector<SNode, String>() {
      public String select(SNode it) {
        return SPropertyOperations.getString(it, PROPS.id$7MjP);
      }
  ...
  public void synchronizeModule(SNode moduleSpec, final SModule module) {
    List<SModel> expectedModels = ListSequence.fromList(SLinkOperations.getChildren(moduleSpec, LINKS.models$h3QT)).select(new ISelector<SNode, SModel>() {
      public SModel select(SNode it) {
        return getOrCreateModel(module, SNodeToNodeAdapter.wrap(it));
      }
    }).toListSequence();
    List<SModel> existingModels = Sequence.fromIterable(((Iterable<SModel>) module.getModels())).toListSequence();
    List<SModel> modelsToRemove = ListSequence.fromList(existingModels).subtract(ListSequence.fromList(expectedModels)).toListSequence();
    List<SModel> modelsToAdd = ListSequence.fromList(expectedModels).subtract(ListSequence.fromList(existingModels)).toListSequence();
    ...
    ListSequence.fromList(modelsToRemove).visitAll(new IVisitor<SModel>() {
  ...
  protected void updateModuleImports(SModule module) {
    Iterable<SModuleReference> moduleRefs = Sequence.fromIterable(Sequence.<SModule>singleton(module)).translate(new ITranslator2<SModule, SModel>() {
      public Iterable<SModel> translate(SModule it) {
        return it.getModels();
      }
    }).ofType(SM_ShadowModel.class).translate(new ITranslator2<SM_ShadowModel, SModelReference>() {
      public Iterable<SModelReference> translate(SM_ShadowModel it) {
  ...
  public void synchronizeModel(SNode modelSpec, final SModel model) {
    Set<SNode> expectedRootNodes = SetSequence.fromSetWithValues(new HashSet<SNode>(), ListSequence.fromList(SLinkOperations.getChildren(modelSpec, LINKS.rootNodes$jxXY)).select(new ISelector<SNode, SNode>() {
      public SNode select(SNode it) {
        return getNode(SNodeToNodeAdapter.wrap(it));
      }
    }));
