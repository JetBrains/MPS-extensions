...
public class SM_ShadowOutputSynchronizerStep1 {
  ...
  protected void synchronizeChildren(INode inputParent, final INode outputParent, final IChildLink link) {
    ...
    List<INode> expectedOutputChildren = ListSequence.fromList(inputChildren).select(new ISelector<INode, INode>() {
      public INode select(INode it) {
        return getOutput(it);
      }
    }).toListSequence();
    ...
    if (!(Objects.equals(existingOutputChildren, expectedOutputChildren))) {
      ListSequence.fromList(existingOutputChildren).visitAll(new IVisitor<INode>() {
      ...
      ListSequence.fromList(expectedOutputChildren).visitAll(new IVisitor<INode>() {
        public void visit(INode it) {
          long childId = ((SM_PNodeAdapter) it).getNodeId();
