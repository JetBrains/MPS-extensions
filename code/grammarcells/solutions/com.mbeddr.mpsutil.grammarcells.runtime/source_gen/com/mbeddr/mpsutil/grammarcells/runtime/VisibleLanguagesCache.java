...
public class VisibleLanguagesCache {
  ...
  public SModuleReference[] getVisibleLanguageReferences(SModel model) {
    ...
    if (result == null) {
      result = SetSequence.fromSet(getVisibleLanguages(model)).select(new ISelector<SLanguage, SModuleReference>() {
        public SModuleReference select(SLanguage it) {
          return it.getSourceModule().getModuleReference();
        }
  ...
  public void invalidate() {
    SetSequence.fromSet(myActiveModels).visitAll(new IVisitor<SModel>() {
      public void visit(SModel it) {
        ((SModelInternal) it).removeModelListener(myModelListener);
      }
    });
    SetSequence.fromSet(myActiveModules).visitAll(new IVisitor<SModule>() {
      public void visit(SModule it) {
        it.removeModuleListener(myModuleListener);
      }
    });
    SetSequence.fromSet(myActiveRepos).visitAll(new IVisitor<SRepository>() {
      public void visit(SRepository it) {
        it.removeRepositoryListener(myRepoListener);
      }
    });
