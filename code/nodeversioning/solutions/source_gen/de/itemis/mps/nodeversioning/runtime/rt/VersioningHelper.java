...
public class VersioningHelper {
  ...
  private static SNode lastVersion(@NotNull SNode n) {
    return ListSequence.fromList(new IAttributeDescriptor.NodeAttribute(CONCEPTS.NodeVersion$qo).list(n)).sort(new ISelector<SNode, Integer>() {
      public Integer select(SNode it) {
        return SPropertyOperations.getInteger(it, PROPS.version$c2Ao);
      }
    }, false).first();
  }
  /**
  ...
  public static SNode newVersion(@NotNull SNode n) {
    ...
    boolean hasCycle = Sequence.fromIterable(dependencies).translate(new ITranslator2<SNode, SNode>() {
      public Iterable<SNode> translate(SNode it) {
        return collectDependencies(it);
      }
    }).contains(n);
    ...
    if (Sequence.fromIterable(dependencies).any(new IWhereFilter<SNode>() {
      public boolean accept(SNode it) {
        return hasNodeChanged(it);
      }
    })) {
    ...
    Iterable<SNode> dependencyVersions = Sequence.fromIterable(dependencies).select(new ISelector<SNode, SNode>() {
      public SNode select(SNode it) {
        return createLinkVersion_uwzupo_a0a0a0a71a8(lastVersion(it));
      }
    });
  ...
  public static List<Tuples._3<SNode, SNode, Boolean>> calculateChanges(@NotNull SNode n, @Nullable SNode since) {
    ...
    for (Tuples._2<SNode, CalculationValue<Tuples._2<Boolean, SNode>>> item : ListSequence.fromList(changeSet)) {
      ...
      Iterable<SNode> dependencyVersions = Sequence.fromIterable(collectDependencies(nodeWithVersion)).select(new ISelector<SNode, SNode>() {
        public SNode select(final SNode dep) {
          return createLinkVersion_uwzupo_a0a0a0a5a9a31(nodeVersionLens(ListSequence.fromList(changeSet).findFirst(new IWhereFilter<Tuples._2<SNode, CalculationValue<Tuples._2<Boolean, SNode>>>>() {
            public boolean accept(Tuples._2<SNode, CalculationValue<Tuples._2<Boolean, SNode>>> it) {
              return Objects.equals(it._0(), dep);
            }
          })));
        }
      });
  ...
  private static boolean calculateChanges(final SNode n, @Nullable SNode since, final List<Tuples._2<SNode, CalculationValue<Tuples._2<Boolean, SNode>>>> calculations) {
    ...
    Tuples._2<SNode, CalculationValue<Tuples._2<Boolean, SNode>>> runningCalculation = ListSequence.fromList(calculations).findFirst(new IWhereFilter<Tuples._2<SNode, CalculationValue<Tuples._2<Boolean, SNode>>>>() {
      public boolean accept(Tuples._2<SNode, CalculationValue<Tuples._2<Boolean, SNode>>> it) {
        return Objects.equals(it._0(), n);
      }
    });
    ...
    boolean hasNodeChanged = ListSequence.fromList(checkResult).any(new IWhereFilter<NodeVersionCheckResult>() {
      public boolean accept(NodeVersionCheckResult it) {
        return !(Objects.equals(it.getChange(), NodeVersionCheckResult.ChangeType.NoChanges));
      }
    });
    ...
    if (!(hasNodeChanged)) {
      ...
      boolean dependencyCurrentlyCalculating = Sequence.fromIterable(dependencies).any(new IWhereFilter<SNode>() {
        public boolean accept(final SNode dependency) {
          Tuples._2<SNode, CalculationValue<Tuples._2<Boolean, SNode>>> calculationForDependency = ListSequence.fromList(calculations).findFirst(new IWhereFilter<Tuples._2<SNode, CalculationValue<Tuples._2<Boolean, SNode>>>>() {
            public boolean accept(Tuples._2<SNode, CalculationValue<Tuples._2<Boolean, SNode>>> it) {
              return Objects.equals(it._0(), dependency);
            }
          });
  ...
  private static SNode getDependencyVersion(SNode version, final SNode dependency) {
    return SLinkOperations.getTarget(ListSequence.fromList(SLinkOperations.getChildren(version, LINKS.dependencies$nOJH)).findFirst(new IWhereFilter<SNode>() {
      public boolean accept(SNode it) {
        return Objects.equals(nodeFromDependency(it), dependency);
      }
    }), LINKS.nodeVersion$nOIJ);
  }
  ...
  public static List<NodeVersionCheckResult> checkNodeVersion(@NotNull SNode n, @Nullable SNode since) {
    ...
    List<SNode> versionedDependencies = ListSequence.fromList(SLinkOperations.getChildren(lastVersion, LINKS.dependencies$nOJH)).select(new ISelector<SNode, SNode>() {
      public SNode select(SNode it) {
        return nodeFromDependency(it);
      }
    }).toListSequence();
    final List<SNode> currentDependencies = Sequence.fromIterable(collectDependencies(n)).toListSequence();
    ...
    for (SNode dep : ListSequence.fromList(SLinkOperations.getChildren(lastVersion, LINKS.dependencies$nOJH)).where(new IWhereFilter<SNode>() {
      public boolean accept(SNode it) {
        return ListSequence.fromList(currentDependencies).contains(nodeFromDependency(it));
      }
    })) {
    ...
    ListSequence.fromList(results).addSequence(Sequence.fromIterable(removedDependencies).select(new ISelector<SNode, NodeVersionCheckResult>() {
      public NodeVersionCheckResult select(SNode it) {
        return NodeVersionCheckResult.dependencyRemoved(it);
      }
    }));
    ListSequence.fromList(results).addSequence(Sequence.fromIterable(newDependencies).select(new ISelector<SNode, NodeVersionCheckResult>() {
      public NodeVersionCheckResult select(SNode it) {
        return NodeVersionCheckResult.dependencyAdded(it);
      }
    }));
  ...
  private static Iterable<SNode> collectDependencies(@NotNull final SNode n) {
    Iterable<SNode> seq = ListSequence.fromList(SNodeOperations.getNodeDescendants(n, CONCEPTS.BaseConcept$gP, false, new SAbstractConcept[]{})).translate(new ITranslator2<SNode, SReference>() {
      public Iterable<SReference> translate(SNode it) {
        return (Iterable<SReference>) it.getReferences();
      }
    }).where(new IWhereFilter<SReference>() {
      public boolean accept(SReference it) {
        SNode sourceNode = ((SNode) it.getSourceNode());
        return !(SNodeOperations.hasRole(sourceNode, LINKS.smodelAttribute$KJ43)) && ListSequence.fromList(SNodeOperations.getNodeAncestors(sourceNode, null, false)).all(new IWhereFilter<SNode>() {
          public boolean accept(SNode it) {
      ...
      public boolean accept(SNode it) {
        return !(it.getModel().isReadOnly());
      }
    }).toListSequence();
    return seq;
  }
  private static boolean isOk(List<NodeVersionCheckResult> results) {
    return ListSequence.fromList(results).all(new IWhereFilter<NodeVersionCheckResult>() {
      public boolean accept(NodeVersionCheckResult it) {
        return Objects.equals(it.getChange(), NodeVersionCheckResult.ChangeType.NoChanges);
      }
    });
