...
public abstract class OptionalPlugin {
  ...
  public boolean isEnabledByDependencies() {
    return SetSequence.fromSet(reverseDependencies).any(new IWhereFilter<OptionalPlugin>() {
      public boolean accept(OptionalPlugin it) {
        return it.isEnabled();
      }
    });
  }
  ...
  public void setEnabledByUser(boolean enabled) {
    ...
    if (!(enabled)) {
      SetSequence.fromSet(reverseDependencies).visitAll(new IVisitor<OptionalPlugin>() {
        public void visit(OptionalPlugin it) {
          it.setEnabledByUser(false);
        }
      });
    }
    applyStateChange();
    SetSequence.fromSet(dependencies).visitAll(new IVisitor<OptionalPlugin>() {
      public void visit(OptionalPlugin it) {
        it.applyStateChange();
      }
    });
  ...
  protected void applyStateChange() {
    if (moduleInititalized) {
      SetSequence.fromSet(dependencies).visitAll(new IVisitor<OptionalPlugin>() {
        public void visit(OptionalPlugin it) {
          it.addReverseDependency(OptionalPlugin.this);
        }
      });
    } else {
      SetSequence.fromSet(dependencies).visitAll(new IVisitor<OptionalPlugin>() {
        public void visit(OptionalPlugin it) {
          it.removeReverseDependency(OptionalPlugin.this);
        }
      });
    }
