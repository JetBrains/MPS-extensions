...
public class SelectionIntentionsManager {
  ...
  public void init() {
    ClassLoaderManager.getInstance().addListener(myClasseslistener);
  ...
  public List<ISelectionIntentionExecutable> getApplicableIntentions(final Selection selection) {
    ...
    selection.getEditorComponent().getEditorContext().getRepository().getModelAccess().runReadAction(() -> {
      result.value = ListSequence.fromList(getIntentions(selection)).where(new IWhereFilter<ISelectionIntentionExecutable>() {
        public boolean accept(ISelectionIntentionExecutable it) {
