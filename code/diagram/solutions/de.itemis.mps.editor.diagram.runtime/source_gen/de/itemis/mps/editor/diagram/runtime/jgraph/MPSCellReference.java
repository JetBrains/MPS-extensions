...
public class MPSCellReference {
  ...
  public MPSCellReference(MPSCell mpsCell) {
    ...
    myIds = ListSequence.fromList(myIds).reversedList().skip(1).toListSequence();
  ...
  public IDiagramElement resolveElement(DiagramModel parent, final Iterable<IAccessorKey> ids) {
    ...
    if (Sequence.fromIterable(ids).count() == 1) {
      ...
      DiagramModel childModel = ListSequence.fromList(childModels).findFirst(new IWhereFilter<DiagramModel>() {
        public boolean accept(DiagramModel it) {
          return Objects.equals(it.getDiagramAccessor().getId(), Sequence.fromIterable(ids).first());
        }
      });
