...
public class ModelSynchronizer {
    ...
    for (Box box : ListSequence.fromListWithValues(new ArrayList<Box>(), myModel.getBoxes())) {
      if (box instanceof PendingBox) {
        ...
        if (pending.canCreate()) {
          ...
          final List<Edge> edgesFrom = Sequence.fromIterable(myModel.getEdges()).where(new IWhereFilter<Edge>() {
            public boolean accept(Edge it) {
              return it.getResolvedEndpointFrom() == pending;
            }
          }).toListSequence();
          final List<Edge> edgesTo = Sequence.fromIterable(myModel.getEdges()).where(new IWhereFilter<Edge>() {
            public boolean accept(Edge it) {
              return it.getResolvedEndpointTo() == pending;
            }
          }).toListSequence();
          ...
          ListSequence.fromList(myWritesAfterRead).addElement(new _FunctionTypes._void_P0_E0() {
            public void invoke() {
              Box newBox = (Box) ListSequence.fromList(myNewElements).findFirst(new IWhereFilter<IDiagramElement>() {
  ...
  protected void fromSModel_(@Nullable final IDiagramCell diagramECell) {
    ...
    ListSequence.fromList(connectionTypes).addSequence(Sequence.fromIterable(connTypesProviders).where(new IWhereFilter<IConnectionTypesExtensionProvider>() {
      public boolean accept(IConnectionTypesExtensionProvider it) {
        return it.appliesFor(SNodeOperations.getConcept(myModel.getSNode()));
      }
    }).translate(new ITranslator2<IConnectionTypesExtensionProvider, IConnectionType>() {
      public Iterable<IConnectionType> translate(IConnectionTypesExtensionProvider it) {
        return it.getConnectionTypes(myModel.getSNode(), myModel.getEditorContext());
      }
    }));
    myModel.setConnectionTypes(ListSequence.fromList(connectionTypes).select(new ISelector<IConnectionType, ConnectionCreatorAdapter>() {
      public ConnectionCreatorAdapter select(IConnectionType it) {
        return new ConnectionCreatorAdapter(it, myModel);
      }
    }).toListSequence());
    ...
    Iterable<IDiagramElement> sortedElements = Sequence.fromIterable(MapSequence.fromMap(myElements).values()).where(new IWhereFilter<IDiagramElement>() {
      public boolean accept(IDiagramElement it) {
        return it instanceof Box;
      }
    });
    sortedElements = Sequence.fromIterable(sortedElements).concat(Sequence.fromIterable(MapSequence.fromMap(myElements).values()).where(new IWhereFilter<IDiagramElement>() {
      public boolean accept(IDiagramElement it) {
        return it instanceof Edge;
      }
    }));
    sortedElements = Sequence.fromIterable(sortedElements).concat(Sequence.fromIterable(MapSequence.fromMap(myElements).values()).where(new IWhereFilter<IDiagramElement>() {
      public boolean accept(IDiagramElement it) {
        return it instanceof Annotation;
      }
    }));
  ...
  public void fromSModel_Box(@NotNull IBoxAccessor accessor, @NotNull Box element) {
    ...
    if (ListSequence.fromList(contentCells).count() == 1 && accessor.getRootEditorCell().getSNode().isInstanceOfConcept(CONCEPTS.NodeAttribute$x2)) {
      element.setContentCells(Sequence.fromIterable(Sequence.<EditorCell>singleton(accessor.getRootEditorCell())).toListSequence());
    } else {
    ...
    element.setNavigationTargets(Sequence.fromIterable(accessor.getNavigationTargets()).where(new IWhereFilter<SNode>() {
