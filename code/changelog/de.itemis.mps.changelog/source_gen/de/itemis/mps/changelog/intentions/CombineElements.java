    ...
    if (selection instanceof WordRangeSelection) {
      ISelectionIntentionExecutable intention = new AbstractSelectionIntentionExecutable<WordRangeSelection>() {
        ...
        public void execute(WordRangeSelection selection) {
          ...
          List<SNode> selectedElements = ListSequence.fromList(selectedNodes).select(new ISelector<SNode, SNode>() {
            public SNode select(SNode it) {
              return SNodeOperations.as(it, CONCEPTS.TextElement$WN);
            }
          }).toListSequence();
          SNode newWord = createCombinedElements_hmrw63_a0c0a(ListSequence.fromList(selectedElements).select(new ISelector<SNode, SNode>() {
            public SNode select(SNode it) {
          ...
          ListSequence.fromList(selectedElements).skip(1).visitAll(new IVisitor<SNode>() {
        ...
        public boolean isApplicable(WordRangeSelection selection) {
          ...
          return ListSequence.fromList(selectedNodes).all(new IWhereFilter<SNode>() {
            public boolean accept(SNode it) {
              return SNodeOperations.isInstanceOf(it, CONCEPTS.TextElement$WN);
            }
