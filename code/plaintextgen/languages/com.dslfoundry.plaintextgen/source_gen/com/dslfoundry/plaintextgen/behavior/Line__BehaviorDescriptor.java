...
public final class Line__BehaviorDescriptor extends BaseBHDescriptor {
  ...
  /*package*/ static int getWidth_id6gwxh6GcBOM(@NotNull SNode __thisNode__) {
    ...
    for (SNode word : ListSequence.fromList(SLinkOperations.getChildren(__thisNode__, LINKS.words$teE6))) {
      sum += (int) ITextGenElement__BehaviorDescriptor.getWidth_id6gwxh6GcBOM.invoke(word);
    }
    return sum;
  }
  /*package*/ static Point getPosition_id4GbnmmUaMp0(@NotNull SNode __thisNode__, int row, int column, CharacterMatrix output) {
    ...
    for (SNode item : ListSequence.fromList(SLinkOperations.getChildren(__thisNode__, LINKS.words$teE6))) {
      max = output.getMaximumPosition(max, ITextGenElement__BehaviorDescriptor.getPosition_id4GbnmmUaMp0.invoke(item, ((int) row), ((int) max.y), output));
    }
    return max;
  }
  /*package*/ static int getEditorViewWidth_id2jBmyzyEzhY(@NotNull SNode __thisNode__, boolean isQueriedNode) {
    ...
    if (ListSequence.fromList(SLinkOperations.getChildren(__thisNode__, LINKS.words$teE6)).isNotEmpty()) {
      for (SNode word : ListSequence.fromList(SLinkOperations.getChildren(__thisNode__, LINKS.words$teE6))) {
        if ((word != null)) {
          sum += (int) ITextGenElement__BehaviorDescriptor.getEditorViewWidth_id2jBmyzyEzhY.invoke(word, ((boolean) false));
        }
      }
    }
    return sum;
