...
public class MergeSameChangeTypes_QuickFix extends QuickFix_Runtime {
  ...
  public void execute(SNode node) {
    MapSequence.fromMap(((Map<SEnumerationLiteral, Integer>) MergeSameChangeTypes_QuickFix.this.getField("countMap")[0])).where(new IWhereFilter<IMapping<SEnumerationLiteral, Integer>>() {
      public boolean accept(IMapping<SEnumerationLiteral, Integer> it) {
        return it.value() > 1;
      }
    }).visitAll(new IVisitor<IMapping<SEnumerationLiteral, Integer>>() {
      public void visit(final IMapping<SEnumerationLiteral, Integer> mapping) {
        ...
        ListSequence.fromList(SLinkOperations.getChildren(((SNode) MergeSameChangeTypes_QuickFix.this.getField("release")[0]), LINKS.sections$GrN9)).where(new IWhereFilter<SNode>() {
          public boolean accept(SNode it) {
            return Objects.equals(SPropertyOperations.getEnum(it, PROPS.type$k0hX), mapping.key());
          }
