...
public class SM_SNodeSynchronizer {
  ...
  public void processReferences() {
    ListSequence.fromList(referencesQueue).visitAll(new IVisitor<ReferenceToResolve>() {
      public void visit(ReferenceToResolve it) {
        it.run();
      }
    });
    ...
    if (Objects.equals(SNodeOperations.getConcept(newNode), SNodeOperations.getConcept(existingNode))) {
      ...
      for (SContainmentLink link : CollectionSequence.fromCollection(concept.getContainmentLinks())) {
        ...
        if (ListSequence.fromList(currentChildren).count() == ListSequence.fromList(newChildren).count()) {
          ...
          ListSequence.fromList(newChildren).translate(new ITranslator2<SNode, SNode>() {
            public Iterable<SNode> translate(SNode it) {
              return SNodeOperations.getNodeDescendants(it, CONCEPTS.BaseConcept$gP, true, new SAbstractConcept[]{});
            }
          }).visitAll(new IVisitor<SNode>() {
            public void visit(SNode it) {
              MapSequence.fromMap(newNodeToActualNode).put(it, it);
            }
          });
        }
      }
      ...
      ListSequence.fromList(SNodeOperations.getNodeDescendants(newNode, CONCEPTS.BaseConcept$gP, true, new SAbstractConcept[]{})).visitAll(new IVisitor<SNode>() {
        public void visit(SNode it) {
          MapSequence.fromMap(newNodeToActualNode).put(it, it);
        }
      });
