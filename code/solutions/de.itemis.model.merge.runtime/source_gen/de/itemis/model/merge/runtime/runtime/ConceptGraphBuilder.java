...
public class ConceptGraphBuilder {
  ...
  private static List<SAbstractConcept> directSubConcepts(List<SAbstractConcept> conceptList, final SAbstractConcept concept) {
    List<SAbstractConcept> transSubConcepts = ListSequence.fromList(conceptList).where(new IWhereFilter<SAbstractConcept>() {
      public boolean accept(SAbstractConcept it) {
        return it.isSubConceptOf(concept) && !(Objects.equals(it, concept));
      }
    }).toListSequence();
    ...
    for (final SAbstractConcept transSubconcept : ListSequence.fromList(transSubConcepts)) {
      ...
      for (SAbstractConcept transSubConcept2 : ListSequence.fromList(transSubConcepts).where(new IWhereFilter<SAbstractConcept>() {
        public boolean accept(SAbstractConcept it) {
          return !(Objects.equals(it, transSubconcept));
        }
      })) {
  ...
  private static <X> Collection<X> directSubConcepts(Collection<X> conceptList, final X concept, final _FunctionTypes._return_P2_E0<? extends Boolean, ? super X, ? super X> subConceptOf) {
    List<X> transSubConcepts = CollectionSequence.fromCollection(conceptList).where(new IWhereFilter<X>() {
      public boolean accept(X it) {
        return subConceptOf.invoke(it, concept) && !(Objects.equals(it, concept));
      }
    }).toListSequence();
    ...
    for (final X transSubconcept : ListSequence.fromList(transSubConcepts)) {
      boolean noTransitive = true;
      for (X transSubConcept2 : ListSequence.fromList(transSubConcepts).where(new IWhereFilter<X>() {
        public boolean accept(X it) {
          return !(Objects.equals(it, transSubconcept));
        }
      })) {
