...
public class TableRangeSelection extends AbstractMultipleSelection {
  ...
  public TableRangeSelection(EditorComponent editorComponent, Map<String, String> properties, CellInfo cellInfo) throws SelectionStoreException, SelectionRestoreException {
    ...
    myStartColumn = SelectionInfoImpl.Util.getIntProperty(properties, PROPERTY_END_COLUMN);
    myStartRow = SelectionInfoImpl.Util.getIntProperty(properties, PROPERTY_START_ROW);
    myEndColumn = SelectionInfoImpl.Util.getIntProperty(properties, PROPERTY_END_COLUMN);
    myEndRow = SelectionInfoImpl.Util.getIntProperty(properties, PROPERTY_END_ROW);
    init();
  }
  ...
  public List<EditorCell> getSelectedGridCells() {
    ...
    for (int x = Math.min(myStartColumn, myEndColumn); x <= Math.max(myStartColumn, myEndColumn); x++) {
      for (int y = Math.min(myStartRow, myEndRow); y <= Math.max(myStartRow, myEndRow); y++) {
        EditorCell cell = check_tr8opa_a0a0a0c0u(as_tr8opa_a0a0a0a0c0u(grid.getElement(x, y), EditorCellGridLeaf.class));
        if (cell != null) {
  ...
  public SelectionInfo getSelectionInfo() throws SelectionStoreException {
    ...
    selectionInfo.getPropertiesMap().put(PROPERTY_START_COLUMN, Integer.toString(myStartColumn));
    selectionInfo.getPropertiesMap().put(PROPERTY_START_ROW, Integer.toString(myStartRow));
    selectionInfo.getPropertiesMap().put(PROPERTY_END_COLUMN, Integer.toString(myEndColumn));
    selectionInfo.getPropertiesMap().put(PROPERTY_END_ROW, Integer.toString(myEndRow));
  ...
  public boolean isSame(Selection other) {
    ...
    if (myEndColumn != that.myEndColumn) {
      return false;
    }
    if (myEndRow != that.myEndRow) {
      return false;
    }
    if (myStartColumn != that.myStartColumn) {
      return false;
    }
    if (myStartRow != that.myStartRow) {
      return false;
    }
  ...
  private static EditorCell check_tr8opa_a0a0a0c0u(EditorCellGridLeaf checkedDotOperand) {
    if (null != checkedDotOperand) {
      return checkedDotOperand.getEditorCell();
    }
    return null;
  }
  private static <T> T as_tr8opa_a0a0a0a0c0u(Object o, Class<T> type) {
