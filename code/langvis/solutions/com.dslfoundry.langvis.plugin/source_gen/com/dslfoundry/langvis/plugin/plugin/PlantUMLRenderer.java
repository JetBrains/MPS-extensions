...
public class PlantUMLRenderer {
    ...
    if (this.collapseTraits && ListSequence.fromList(SLinkOperations.getChildren(decl, LINKS.propertyDeclaration$YUgg)).isEmpty()) {
      {
        ...
        if (SNodeOperations.isInstanceOf(idecl, CONCEPTS.InterfaceConceptDeclaration$CG)) {
          return ListSequence.fromList(SLinkOperations.getChildren(idecl, LINKS.extends$nawU)).all(new IWhereFilter<SNode>() {
            public boolean accept(SNode it) {
              return PlantUMLRenderer.this.isTrait(SLinkOperations.getTarget(it, LINKS.intfc$zM4e));
            }
          });
        }
      }
      {
        ...
        if (SNodeOperations.isInstanceOf(cdecl, CONCEPTS.ConceptDeclaration$gH)) {
          return SConceptOperations.isExactly(SNodeOperations.asSConcept(SNodeOperations.getConcept(SLinkOperations.getTarget(cdecl, LINKS.extends$_Isg))), CONCEPTS.BaseConcept$gP) && ListSequence.fromList(SLinkOperations.getChildren(cdecl, LINKS.implements$u_P2)).all(new IWhereFilter<SNode>() {
            public boolean accept(SNode it) {
              return PlantUMLRenderer.this.isTrait(SLinkOperations.getTarget(it, LINKS.intfc$zM4e));
            }
          });
  ...
  public List<SNode> inheritors(SNode base) {
    ...
    return ListSequence.fromList(SConceptOperations.getAllSubConcepts(c, SNodeOperations.getModel(base))).select(new ISelector<SAbstractConcept, SNode>() {
      public SNode select(SAbstractConcept it) {
        return SNodeOperations.cast(SNodeOperations.asNode(it), CONCEPTS.AbstractConceptDeclaration$KA);
      }
    }).toListSequence();
  }
  ...
  public void renderPlantUMLSource(Set<SNode> elements, boolean renderChildren, boolean renderParents, boolean renderCardinalities, boolean renderRoleNames, boolean flattenNamespaces, String outFile) throws IOException {
    ...
    final List<SNode> interfaces = Sequence.fromIterable(SNodeOperations.ofConcept(elements, CONCEPTS.InterfaceConceptDeclaration$CG)).toListSequence();
    final List<SNode> interfacesBoxes = ListSequence.fromList(interfaces).where(new IWhereFilter<SNode>() {
      public boolean accept(SNode it) {
        return showAsBox(it);
      }
    }).toListSequence();

    final List<SNode> concepts = Sequence.fromIterable(SNodeOperations.ofConcept(elements, CONCEPTS.ConceptDeclaration$gH)).toListSequence();
    final List<SNode> conceptsBoxes = ListSequence.fromList(concepts).where(new IWhereFilter<SNode>() {
      public boolean accept(SNode it) {
        return showAsBox(it);
      }
    }).toListSequence();
    ...
    try {
      ...
      if (renderParents) {
        for (SNode intf : ListSequence.fromList(interfaces)) {
          // Interface extension
          for (SNode implementedInterface : ListSequence.fromList(SLinkOperations.getChildren(intf, LINKS.extends$nawU)).where(new IWhereFilter<SNode>() {
            public boolean accept(SNode it) {
              return showAsBox(SLinkOperations.getTarget(it, LINKS.intfc$zM4e));
            }
          })) {
        ...
        for (SNode concept : ListSequence.fromList(concepts)) {
          ...
          for (SNode implementedInterface : ListSequence.fromList(SLinkOperations.getChildren(concept, LINKS.implements$u_P2)).where(new IWhereFilter<SNode>() {
            public boolean accept(SNode it) {
              return showAsBox(SLinkOperations.getTarget(it, LINKS.intfc$zM4e));
            }
          })) {
  ...
  public void collectDownUp(SNode node, Set<SNode> collectedNodes) {
    ...
    List<SNode> filter = SetSequence.fromSet(collectedNodes).where(new IWhereFilter<SNode>() {
      public boolean accept(SNode it) {
        return !(showAsBox(it));
      }
    }).toListSequence();
  ...
  private String makeStereotypeList(SNode extendedConcept, List<SNode> Interfaces, boolean alwaysAsAnnotation) {
    ...
    if (alwaysAsAnnotation) {
      annotations = ListSequence.fromList(Interfaces).select(new ISelector<SNode, String>() {
        public String select(SNode it) {
          return SPropertyOperations.getString(SLinkOperations.getTarget(it, LINKS.intfc$zM4e), PROPS.name$MnvL);
        }
      }).toListSequence();
      ...
      annotations = ListSequence.fromList(Interfaces).where(new IWhereFilter<SNode>() {
        public boolean accept(SNode it) {
          return showAsAnnotation(SLinkOperations.getTarget(it, LINKS.intfc$zM4e));
        }
