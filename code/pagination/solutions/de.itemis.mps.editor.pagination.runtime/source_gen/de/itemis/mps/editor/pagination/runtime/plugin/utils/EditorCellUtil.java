...
public class EditorCellUtil {
  ...
  public static Iterable<EditorCell> getAncestors(final EditorCell cell, boolean includeStart) {
    ...
    return Sequence.fromIterable(initialCells).concat(Sequence.fromIterable(Sequence.fromClosure(new ISequenceClosure<EditorCell>() {
      public Iterable<EditorCell> iterable() {
        return getAncestors(cell.getParent(), true);
      }
    })));
  }
...
__switch__:
      ...
      public Iterable<EditorCell> translate(EditorCell childEditorCell) {
        return getDescendants(childEditorCell, true);
      }
    }));
  }
  ...
  public static EditorComponent openEditorComponentForNode(SNode node, Project project) {
    return check_eu2c9k_a0a11(NavigationSupport.getInstance().openNode(project, node, true, true));
  }

  public static Iterable<EditorCell> getNodesWithCell(Iterable<SNode> nodeSeq, final EditorComponent editorComponent) {
    return Sequence.fromIterable(nodeSeq).select(new ISelector<SNode, EditorCell>() {
      public EditorCell select(SNode n) {
        return editorComponent.findNodeCell(n);
      }
    }).where(new NotNullWhereFilter<EditorCell>());
  }
  ...
  private static EditorComponent check_eu2c9k_a0a11(Editor checkedDotOperand) {
    if (null != checkedDotOperand) {
