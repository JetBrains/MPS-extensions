...
public abstract class AbstractLineList implements ILineList {
  ...
  public ILineList merge(ILineList other) {
    Iterable<Line> allLines = Sequence.fromIterable(getLines()).concat(Sequence.fromIterable(other.getLines())).sort(new ISelector<Line, Integer>() {
      public Integer select(Line it) {
        return it.getMinY();
      }
    }, true).alsoSort(new ISelector<Line, Integer>() {
      public Integer select(Line it) {
        return it.getMaxY();
      }
    }, true);
