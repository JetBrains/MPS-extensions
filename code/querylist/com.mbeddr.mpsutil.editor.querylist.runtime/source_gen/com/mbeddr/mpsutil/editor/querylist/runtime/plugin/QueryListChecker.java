...
public class QueryListChecker extends BaseEditorChecker {
  ...
  public UpdateResult update(final EditorComponent editorComponent, final boolean incremental, final boolean applyQuickFixes, final Cancellable cancellable) {
    ...
    final List<SModel> modelsToCheck = SetSequence.fromSet(nodesToCheck).select(new ISelector<SNode, SModel>() {
  ...
  private Set<SNode> collectNodesToCheck(EditorCell rootCell) {
    ...
    return SetSequence.fromSetWithValues(new HashSet<SNode>(), Sequence.fromIterable(queryListCells).translate(new ITranslator2<EditorCell_QueryList, SNode>() {
  ...
  private List<IChecker<?, ? extends IssueKindReportItem>> getCheckers(@Nullable ComponentHost host) {
    List<IChecker<?, ?>> standardCheckers = (List<IChecker<?, ?>>) NodeCheckerUtil.getStandardCheckers(host);
    IChecker<?, ? extends IssueKindReportItem> nonTypesystemChecker = ListSequence.fromList(standardCheckers).findFirst(new IWhereFilter<IChecker<?, ?>>() {
      public boolean accept(IChecker<?, ?> it) {
        return it instanceof NonTypesystemChecker;
      }
    });
    ...
    return (List<IChecker<?, ? extends IssueKindReportItem>>) standardCheckers;
  }
