...
public class NamedScope extends AbstractScope implements IScope {
  public static IScope concat(Iterable<IScope> scopes) {
    return Sequence.fromIterable(scopes).reduceLeft(new ILeftCombinator<IScope, IScope>() {
      public IScope combine(IScope a, IScope b) {
  ...
  public NamedScope(IScope parent, String name, ParameterList parameters, Iterable<ScopeImport> imports) {
    ...
    this.imports = ListSequence.fromListWithValues(new ArrayList<ScopeImport>(), Sequence.fromIterable(imports).where(new NotNullWhereFilter<ScopeImport>()));
  }
