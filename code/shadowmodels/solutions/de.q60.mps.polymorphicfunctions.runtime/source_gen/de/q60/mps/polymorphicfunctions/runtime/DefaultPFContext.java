...
public class DefaultPFContext extends AbstractPFContext implements IPFContext {
  ...
  public Iterable<Object> callFunctions(String id, ParameterList parameters, IPFContext context) {
    return Sequence.fromIterable(callFunctions(id, parameters, true, context)).toListSequence();
  }
  ...
  protected Iterable<Object> callFunctions(final String id, final ParameterList parameters, final boolean multiple, final IPFContext context) {
    ...
    return ListSequence.fromList(applicableImplementations).select(new ISelector<IFunctionImplementation, Object>() {
      public Object select(final IFunctionImplementation it) {
        return IPFContext.CURRENT.computeWith(context, () -> myRecursionProtection.evaluate(it, parameters, context));
      }
    });
  }
  ...
  protected List<IFunctionImplementation> getApplicableImplementations(final String id, final ParameterList parameters, final IPFContext context) {
    ...
    result = ListSequence.fromList(myImplementationProviders).translate(new ITranslator2<IImplementationProvider, IFunctionImplementation>() {
      public Iterable<IFunctionImplementation> translate(IImplementationProvider it) {
        return it.getImplementations(context, id, parameters);
      }
    });
    result = Sequence.fromIterable(result).where(new IWhereFilter<IFunctionImplementation>() {
      public boolean accept(IFunctionImplementation it) {
        return Sequence.fromIterable(it.getContextIds()).all(new IWhereFilter<String>() {
    ...
    result = Sequence.fromIterable(result).where(new IWhereFilter<IFunctionImplementation>() {
      public boolean accept(IFunctionImplementation it) {
        return it.isApplicable(context, parameters);
      }
    }).toListSequence();
