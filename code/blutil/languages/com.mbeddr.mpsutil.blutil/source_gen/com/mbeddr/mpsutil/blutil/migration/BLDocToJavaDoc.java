...
public class BLDocToJavaDoc extends MigrationScriptBase {
  ...
  public void doExecute(final SModule m) {
    ...
    for (SModel model : models) {
      ListSequence.fromList(SModelOperations.nodes(model, CONCEPTS.BLDoc$UI)).where(new IWhereFilter<SNode>() {
        public boolean accept(SNode it) {
          return !(TestCaseDetection.isInMigrationTestCase(it)) && ListSequence.fromList(SLinkOperations.getChildren(SLinkOperations.getTarget(it, LINKS.text$Lu49), LINKS.words$C8QZ)).all(new IWhereFilter<SNode>() {
            public boolean accept(SNode it) {
  ...
  /*package*/ Iterable<SNode> createComments(Iterable<String> lines) {
    return Sequence.fromIterable(lines).select(new ISelector<String, SNode>() {
      public SNode select(String it) {
        return createCommentLine_ra7ppi_a0a0a0a0k(it);
      }
    });
  }
