...
public class LineList extends AbstractLineList implements ILineList {
  ...
  public static ILineList create(Iterable<ILineList> subLists_) {
    List<ILineList> subLists = Sequence.fromIterable(subLists_).toListSequence();
  ...
  public LineList(Iterable<ILineList> subLists) {
    this.subLists = Sequence.fromIterable(subLists).sort(new ISelector<ILineList, Integer>() {
    ...
    }, true).toGenericArray(ILineList.class);
    subListForMinY = Sequence.fromIterable(subLists).sort(new ISelector<ILineList, Integer>() {
      public Integer select(ILineList it) {
        return it.getMinY();
      }
    }, true).first();
    subListForMaxY = Sequence.fromIterable(subLists).sort(new ISelector<ILineList, Integer>() {
      public Integer select(ILineList it) {
        return it.getMaxY();
      }
    }, false).first();
    this.size = Sequence.fromIterable(Sequence.fromArray(this.subLists)).foldLeft(0, new ILeftCombinator<ILineList, Integer>() {
      public Integer combine(Integer s, ILineList it) {
        return s + it.getSize();
      }
    });
  ...
  public Iterable<Line> getLines() {
    return Sequence.fromIterable(Sequence.fromArray(subLists)).translate(new ITranslator2<ILineList, Line>() {
      public Iterable<Line> translate(ILineList it) {
        return it.getLines();
      }
    });
  }
  ...
  public ILineList linesBelowY(final int y) {
    List<ILineList> filteredLines = Sequence.fromIterable(Sequence.fromArray(subLists)).select(new ISelector<ILineList, ILineList>() {
      public ILineList select(ILineList it) {
        return it.linesBelowY(y);
      }
