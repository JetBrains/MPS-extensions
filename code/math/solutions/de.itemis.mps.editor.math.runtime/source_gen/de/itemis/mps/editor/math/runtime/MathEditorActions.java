...
public class MathEditorActions {
  private static final Logger LOG = Logger.getLogger(MathEditorActions.class);
  ...
  public static int formularDistance(EditorCell cell1, EditorCell cell2) {
    ...
    int distance = ListSequence.fromList(getParents(cell1, commonParent)).where(new IWhereFilter<EditorCell>() {
      public boolean accept(EditorCell it) {
        return it instanceof EditorCell_MathBase;
      }
    }).count();
    distance += ListSequence.fromList(getParents(cell2, commonParent)).where(new IWhereFilter<EditorCell>() {
      public boolean accept(EditorCell it) {
        return it instanceof EditorCell_MathBase;
      }
    }).count();
