...
public class LineNumberComponent extends AbstractLeftColumn {
  ...
  public void paint(final Graphics graphics) {
    ...
    final Font font = getEditorComponent().getEditorComponentSettings().getDefaultFont();
    graphics.setFont(font);
    graphics.setColor(Color.LIGHT_GRAY);
    if (lines != null) {
      Sequence.fromIterable(lines.getLines()).visitAll(new IVisitor<Line>() {
        public void visit(Line it) {
          FontMetrics fontMetrics = getEditorComponent().getFontMetrics(font);
          // calculate the x-coordinate to draw the first letter of the line number in dependence of the position of the gutter
          int posx = (isRightSideOfEditor() ? getLeftEditorHighlighter().getVisibleRect().width - getWidth() : getWidth() - fontMetrics.stringWidth(String.valueOf(it.getNumber())));
          graphics.drawString(String.valueOf(it.getNumber()), posx, it.getCell().getY() + (it.getCell().getHeight() - fontMetrics.getHeight()) / 2 + fontMetrics.getAscent());
        }
      });
    }
  }
  ...
  public void relayout(boolean updateFolding) {
    ...
    if (lastUpdated != -1) {
      ...
      if (timeDiffInMS < RELAYOUT_THROTTLE_INTERVAL) {
        return;
      }
    }
