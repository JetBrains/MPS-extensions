...
public class GrammarCellsUtil {
  ...
  public static final String INSERT_TRANSFORMATION_TEXT = "𐂂𐂅𐃍";
  ...
  public static List<IGrammarActionsDescriptor> getVisibleDescriptors(SModel currentModel) {
    ...
    return SetSequence.fromSet(langs).select(new ISelector<SLanguage, IGrammarActionsDescriptor>() {
      public IGrammarActionsDescriptor select(SLanguage it) {
        return getDescriptor(it.getSourceModule());
      }
    }).where(new IWhereFilter<IGrammarActionsDescriptor>() {
      public boolean accept(IGrammarActionsDescriptor it) {
        return it != null;
      }
    }).toListSequence();
  }
  ...
  public static List<SConcept> getVisibleSubconceptsNonAbstract(SAbstractConcept concept, Set<SLanguage> visibleLanguages) {
    ...
    return ListSequence.fromList(allSubConcepts).where(new IWhereFilter<SAbstractConcept>() {
  ...
  public static List<SConcept> getVisibleSubconceptsNonAbstract(SAbstractConcept concept, SModel contextModel, final Class<? extends ConceptEditor> ifEditorNotOveridden, EditorContext editorContext) {
    ...
    return ListSequence.fromList(getVisibleSubconceptsNonAbstract(concept, getVisibleLanguages(contextModel))).where(new IWhereFilter<SConcept>() {
      public boolean accept(SConcept it) {
        return EditorHierachyCache.getInstance().isActiveEditor(ifEditorNotOveridden, it, hints);
      }
    }).toListSequence();
  }
  ...
  public static SNode findRootExpression(SNode start, final SAbstractConcept expressionConcept) {
    return ListSequence.fromList(SNodeOperations.getNodeAncestors(start, null, true)).findFirst(new IWhereFilter<SNode>() {
      public boolean accept(SNode it) {
        return (SNodeOperations.getParent(it) == null) || !(SConceptOperations.isSubConceptOf(SNodeOperations.asSConcept(SNodeOperations.getConcept(SNodeOperations.getParent(it))), SNodeOperations.asSConcept(expressionConcept)));
      }
    });
  }
  ...
  public static boolean isValidPropertyValue(SProperty property, @Nullable SAbstractConcept concept, String value) {
    ...
    return ModelConstraints.validatePropertyValue(dummyNode, property, internalValue, null);
  }
  ...
  public static SAbstractConcept getSpecializedTargetConcept(SContainmentLink genuineLink, SAbstractConcept owner) {
    ...
    SNode specializedLink = ListSequence.fromList(AbstractConceptDeclaration__BehaviorDescriptor.getLinkDeclarations_idhEwILKK.invoke(conceptDecl)).findFirst(new IWhereFilter<SNode>() {
      public boolean accept(SNode it) {
        return SLinkOperations.getTarget(it, LINKS.specializedLink$7ZCN) == linkDecl;
      }
    });
  ...
  public static Iterable<EditorCell> getUnwrappedChildren(EditorCell parent) {
    if (parent instanceof EditorCell_Collection) {
      ...
      return Sequence.fromIterable(children).translate(new ITranslator2<EditorCell, EditorCell>() {
        public Iterable<EditorCell> translate(EditorCell child) {
