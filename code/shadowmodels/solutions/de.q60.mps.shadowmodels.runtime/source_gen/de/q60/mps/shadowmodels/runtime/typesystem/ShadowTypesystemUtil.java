...
public class ShadowTypesystemUtil {
  ...
  public static SNode getType(final SNode node) {
    ...
    MPSModuleRepository.getInstance().getModelAccess().runReadAction(() -> type.value = new TargetTypecheckingContext(node, typeChecker).getTypeOf_generationMode(ANode.wrap(node)));
    if (type.value == null) {
      if (LOG.isDebugLevel()) {
