...
public class LineNumberUtils {
  ...
  private static ILineList findLines_(EditorCell cell, final boolean ignoreCache) {
    ...
    if (cell instanceof EditorCell_Collection) {
      ...
      List<ILineList> mergedLists = mergeOverlapping(Sequence.fromIterable(visibleChildren).select(new ISelector<EditorCell, ILineList>() {
        public ILineList select(EditorCell it) {
          return findLines(it, ignoreCache);
        }
      }).where(new NotNullWhereFilter<ILineList>()).toListSequence());
  ...
  private static int getExpansionDelta(EditorCell cell) {
    if (cell instanceof jetbrains.mps.openapi.editor.cells.EditorCell_Collection) {
      ...
      if (delta == null) {
        ...
        delta = Sequence.fromIterable(children).foldLeft(0, new ILeftCombinator<EditorCell, Integer>() {
          public Integer combine(Integer s, EditorCell it) {
            return s + getExpansionDelta(it);
          }
        });
  ...
  public static List<ILineList> mergeOverlapping(List<ILineList> lists) {
    ...
    for (ILineList list : ListSequence.fromList(lists).sort(new ISelector<ILineList, Integer>() {
      public Integer select(ILineList it) {
        return it.getMinY();
      }
    }, true).alsoSort(new ISelector<ILineList, Integer>() {
      public Integer select(ILineList it) {
        return it.getMaxY();
      }
    }, true)) {
