...
public class PortLayouter {
  ...
  public static void layoutPortCells(BoxDCell parentCell) {
    new PortLayouter(Sequence.fromIterable(parentCell.getPorts()).toListSequence(), parentCell).layoutPortCells();
  }
  ...
  public PortLayouter(List<PortDCell> ports, BoxDCell parentCell) {
    this.ports = ListSequence.fromList(ports).sort((PortDCell a, PortDCell b) -> comparator.invoke(a, b), true).toListSequence();
  ...
  private boolean setPortsPositionIfAllOverlap(List<PortDCell> ports, double size, boolean horizontal) {
    ...
    if (horizontal) {
      ...
      boolean b = ListSequence.fromList(ports).tailListSequence(1).any(new IWhereFilter<PortDCell>() {
        public boolean accept(PortDCell it) {
          return relativeX != it.getPort().getRelativeXNN();
        }
      });
      if (b) {
        return false;
      ...
      boolean b = ListSequence.fromList(ports).tailListSequence(1).any(new IWhereFilter<PortDCell>() {
        public boolean accept(PortDCell it) {
          return relativeY != it.getPort().getRelativeYNN();
        }
      });
      if (b) {
        return false;
