...
/*package*/ class SimpleTableWithoutHeaders_Table_EditorBuilder_a extends AbstractEditorBuilder {
  @NotNull
  private SNode myNode;

  public SimpleTableWithoutHeaders_Table_EditorBuilder_a(@NotNull EditorContext context, @NotNull SNode node) {
    super(context);
    myNode = node;
  }
    ...
    _FunctionTypes._void_P0_E0 creator = () -> {
      CellCaching.maybeDisableCache(node, editorContext, () -> {
        try {
          ...
          final Grid childGrid = createChildsVertical_315fwh_a0(editorContext, node);
          ...
          editorCell.value.setCellId("Table_315fwh_a");
  ...
  public Grid createChildsVertical_315fwh_a0(final EditorContext editorContext, final SNode node) {
    Grid grid = new Grid();
    GridAdapter gridAdapter = new GridAdapter(grid, editorContext, node);


    final IHeaderNodeInsertAction insertAction = new ChildNodesInsertAction(node, SLinkOperations.findLinkDeclaration(LINKS.rows$OqF7)) {};
    ...
    try {
      ...
      int y = 0;
      Iterable<SNode> elements = SLinkOperations.getChildren(node, LINKS.rows$OqF7);
      for (SNode child : Sequence.fromIterable(elements)) {
        ...
        SubstituteInfo substituteInfo = SubstituteUtil.forChild(editorContext, node, (y < ListSequence.fromList(SLinkOperations.getChildren(node, LINKS.rows$OqF7)).count() ? ListSequence.fromList(SLinkOperations.getChildren(node, LINKS.rows$OqF7)).getElement(y) : null), LINKS.rows$OqF7);
    ...
    if (ListSequence.fromList(SLinkOperations.getChildren(node, LINKS.rows$OqF7)).isEmpty()) {
      ...
      SubstituteInfo substituteInfo = new DefaultSChildSubstituteInfo(node, null, LINKS.rows$OqF7, editorContext);
