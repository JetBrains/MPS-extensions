...
public class ButtonsMouseListener implements MouseListener, MouseMotionListener {
  ...
  private static final StyleAttribute<_FunctionTypes._return_P1_E0<? extends Double, ? super Double>> TREE_BUTTON_OPACITY = StyleAttributes.getInstance().getAttribute("com.mbeddr.mpsutil.treenotation.styles", "tree-button-opacity");
  ...
  protected Button getCellAt(final MouseEvent event) {
    return SetSequence.fromSet(myButtons).findFirst(new IWhereFilter<Button>() {
      public boolean accept(Button it) {
        return new Rectangle(it.getX(), it.getY(), it.getWidth(), it.getHeight()).contains(event.getX(), event.getY());
      }
    });
  }
  ...
  public void mouseExited(MouseEvent event) {
    SetSequence.fromSet(myButtons).visitAll(new IVisitor<Button>() {
