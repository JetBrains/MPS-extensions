...
public class AlignBottomButton extends AlignButton {
  public AlignBottomButton(double size, mxCellState state) {
    super(size, state);
  }
  public AlignBottomButton(double size, mxCellState state, double spacing) {
  ...
  protected void paintSymbol(Graphics2D g) {
    ...
    g.draw(new Line2D.Double(ExtensionMethods.toPoint2D(bounds.getLeftBottom()), ExtensionMethods.toPoint2D(bounds.getRightBottom())));
  }

  @Override
  protected void doAlign(BaseDCell cell, BaseDCell otherCell) {
    Bounds bounds = CellExtensions.getAbsoluteBounds(otherCell);
    bounds.setBottom(CellExtensions.getAbsoluteBounds(cell).getMaxY());
