...
public class MigrateOldConfigs extends MigrationScriptBase {
  ...
  public void doExecute(final SModule m) {
    for (SModel smodel : Sequence.fromIterable(m.getModels())) {
      ...
      ListSequence.fromList(SModelOperations.nodes(model, CONCEPTS.DiagramLayoutConfig$Y0)).visitAll(new IVisitor<SNode>() {
      ...
      ListSequence.fromList(SModelOperations.nodes(model, CONCEPTS.LayeredLayoutConfig$Xm)).visitAll(new IVisitor<SNode>() {
        public void visit(SNode it) {
          List<SProperty> properties = ListSequence.fromListWithValues(new ArrayList<SProperty>(), it.getProperties());
          SProperty edgeRoutingOldProperty = ListSequence.fromList(properties).findFirst(new IWhereFilter<SProperty>() {
            public boolean accept(SProperty it) {
              return Objects.equals(it.getName(), PROPS.edgeRoutingOld$NRI4);
            }
          });
          SProperty hierarchyHandlingOldProperty = ListSequence.fromList(properties).findFirst(new IWhereFilter<SProperty>() {
            public boolean accept(SProperty it) {
