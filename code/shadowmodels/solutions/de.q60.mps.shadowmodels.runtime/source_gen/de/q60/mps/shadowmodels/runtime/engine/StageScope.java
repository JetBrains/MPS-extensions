...
public class StageScope extends AbstractScope implements IScope {
  ...
  public Iterable<ScopeImport> getImports(final ITransformationEngine engine) {
    return engine.getIncrementalEngine().evaluate(new CK_StageScope_getImports(this), () -> {
      ...
      imports = Sequence.fromIterable(imports).concat(Sequence.fromIterable(findSiblings(engine)).select(new ISelector<IScope, ScopeImport>() {
        public ScopeImport select(IScope it) {
          return new ScopeImport(it);
        }
      }));
      return Sequence.fromIterable(imports).toListSequence();
    });
  }
  ...
  protected Iterable<IScope> findSiblings(final ITransformationEngine engine) {
    ...
    Iterable<IUniqueContainmentTarget> allSubgraphs = Sequence.fromIterable(parentStage.getAllContent()).where(new IWhereFilter<IUniqueContainmentTarget>() {
      public boolean accept(IUniqueContainmentTarget it) {
        return it.getTarget() instanceof ISubgraphCall;
      }
    });
    Iterable<IUniqueContainmentTarget> siblingsSubgraphs = Sequence.fromIterable(allSubgraphs).where(new IWhereFilter<IUniqueContainmentTarget>() {
      public boolean accept(IUniqueContainmentTarget it) {
        return !(Objects.equals(it.getTarget(), ownCall)) && areSiblings(ownCall, (ISubgraphCall) it.getTarget());
      }
    });
    Iterable<IStageReference> siblingStageRefs = Sequence.fromIterable(siblingsSubgraphs).select(new ISelector<IUniqueContainmentTarget, IStageReference>() {
      public IStageReference select(IUniqueContainmentTarget it) {
    ...
    return Sequence.fromIterable(siblingStageRefs).select(new ISelector<IStageReference, StageScope>() {
      public StageScope select(IStageReference it) {
        return new StageScope(it);
      }
    });
  }
