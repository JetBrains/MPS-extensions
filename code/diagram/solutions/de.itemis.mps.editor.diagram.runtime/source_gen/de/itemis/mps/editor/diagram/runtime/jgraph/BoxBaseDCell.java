...
public abstract class BoxBaseDCell extends BaseDCell implements ISelfPaintingCell, IMPSCellContainer, IDiagramDCell {
  ...
  public void setContentCells(List<EditorCell> contentCells) {
    myMPSCells = ListSequence.fromList(contentCells).where(new IWhereFilter<EditorCell>() {
      public boolean accept(EditorCell it) {
        return it != null;
      }
    }).select(new ISelector<EditorCell, MPSCell>() {
      public MPSCell select(EditorCell it) {
        return myGraph.get().getMPSCellRegistry().getInstance((jetbrains.mps.nodeEditor.cells.EditorCell) it, BoxBaseDCell.this);
      }
    }).toListSequence();
  }
