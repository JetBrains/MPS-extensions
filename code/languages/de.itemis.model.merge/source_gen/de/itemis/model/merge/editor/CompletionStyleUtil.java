...
public class CompletionStyleUtil {
  ...
  public static void apply(CompletionItemInformation itemInformation, EditorMenuItemStyle style, SNode childPolicy) {
    List<SNode> alreadyDefined = ListSequence.fromList(SLinkOperations.getChildren((SNodeOperations.getNodeAncestor(childPolicy, CONCEPTS.MergePolicy$RB, true, false)), LINKS.childPolicies$7ocQ)).select(new ISelector<SNode, SNode>() {
  ...
  public static void apply(CompletionItemInformation itemInformation, EditorMenuItemStyle style, Collection<SNode> itemPolicies) {
    Iterable<SNode> linkDeclarations = CollectionSequence.fromCollection(itemPolicies).select(new ISelector<SNode, SNode>() {
      public SNode select(SNode it) {
        return (SNode) ItemPolicy__BehaviorDescriptor.childLink_id1VmHfRx_0K2.invoke(it);
      }
    });

    List<SNode> alreadyDefined = Sequence.fromIterable(linkDeclarations).where(new IWhereFilter<SNode>() {
      public boolean accept(SNode it) {
        return (it != null);
      }
    }).toListSequence();
  ...
  public static void setStyle(List<SNode> alreadyDefined, CompletionItemInformation itemInformation, EditorMenuItemStyle style) {
    Object parameterObject = itemInformation.getParameterObject();
    if (parameterObject instanceof SNode) {
      SNode n = ((SNode) parameterObject);
      if (alreadyDefined.contains(n)) {
