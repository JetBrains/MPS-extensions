...
public class HeaderGridFactory {
  ...
  private static Iterable<EditorCell> descendantsOfSameNode(EditorCell cell) {
    ...
    if (cell instanceof EditorCell_Collection) {
      ...
      result = Sequence.fromIterable(result).concat(Sequence.fromIterable(children).where(new IWhereFilter<EditorCell>() {
        public boolean accept(EditorCell it) {
          return it.getSNode() == it.getParent().getSNode();
        }
      }).translate(new ITranslator2<EditorCell, EditorCell>() {
        public Iterable<EditorCell> translate(EditorCell it) {
          return descendantsOfSameNode(it);
        }
      }));
    }
    return result;
