...
public class AlignLeftButton extends AlignButton {
  public AlignLeftButton(double size, mxCellState state) {
    super(size, state);
  }
  ...
  protected void paintSymbol(Graphics2D g) {
    ...
    g.draw(new Line2D.Double(ExtensionMethods.toPoint2D(bounds.getLeftTop()), ExtensionMethods.toPoint2D(bounds.getLeftBottom())));
  }

  @Override
  protected void doAlign(BaseDCell cell, BaseDCell otherCell) {
    Bounds bounds = CellExtensions.getAbsoluteBounds(otherCell);
    bounds.setX(CellExtensions.getAbsoluteBounds(cell).getX());
