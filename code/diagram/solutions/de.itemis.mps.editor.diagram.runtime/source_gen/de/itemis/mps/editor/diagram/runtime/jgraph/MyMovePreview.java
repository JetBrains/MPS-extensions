...
public class MyMovePreview extends mxMovePreview {
  ...
  private void expandSubdiagrams(Object[] movedCells) {
    Iterable<SubDiagramDCell> subdiagrams = Sequence.fromIterable(Sequence.fromArray(movedCells)).ofType(BoxBaseDCell.class).select(new ISelector<BoxBaseDCell, mxICell>() {
      public mxICell select(BoxBaseDCell it) {
        return it.getParent();
      }
    }).ofType(SubDiagramDCell.class);
    for (SubDiagramDCell subdiagram : Sequence.fromIterable(subdiagrams)) {
      ...
      Bounds childrenBounds = ListSequence.fromList(CellExtensions.getChildren(subdiagram)).select(new ISelector<mxICell, Bounds>() {
        public Bounds select(mxICell it) {
  ...
  public Tuples._2<Double, Double> snap(double dx, double dy, MouseEvent event) {
    ...
    snapResults = ListSequence.fromList(snapResults).where(new IWhereFilter<SnapResult>() {
      public boolean accept(SnapResult it) {
        return it.getSnapDistance() <= SNAP_DISTANCE_LIMIT;
      }
    }).toListSequence();

    ListSequence.fromList(snapResults).visitAll(new IVisitor<SnapResult>() {
      public void visit(SnapResult it) {
        it.setNumCrossingBoxes(numCrossingBoxes(it, states));
      }
    });

    Iterable<SnapResult> sortedResults = snapResults;
    sortedResults = Sequence.fromIterable(sortedResults).sort(new ISelector<SnapResult, Integer>() {
      public Integer select(SnapResult it) {
        return it.getNumCrossingBoxes();
      }
    }, true);
    sortedResults = Sequence.fromIterable(sortedResults).alsoSort(new ISelector<SnapResult, Integer>() {
      public Integer select(SnapResult it) {
        return it.getPriority();
      }
    }, true);
    sortedResults = Sequence.fromIterable(sortedResults).alsoSort(new ISelector<SnapResult, Double>() {
      public Double select(SnapResult it) {
        return it.getSnapDistance() + it.getLineLength() / 200.0;
      }
    }, true);
    sortedResults = Sequence.fromIterable(sortedResults).alsoSort(new ISelector<SnapResult, Double>() {
      public Double select(SnapResult it) {
        return it.getLineLength();
      }
    }, true);
    ...
    if (bestSnapResult != null) {
      ...
      if (snappedX == 0.0) {
        secondary = Sequence.fromIterable(sortedResults).findFirst(new IWhereFilter<SnapResult>() {
          public boolean accept(SnapResult it) {
            return it.getSnapDistanceX() != 0.0 && it.getSnapDistanceY() == 0.0;
          }
        });
      } else if (snappedY == 0.0) {
        secondary = Sequence.fromIterable(sortedResults).findFirst(new IWhereFilter<SnapResult>() {
          public boolean accept(SnapResult it) {
            return it.getSnapDistanceY() != 0.0 && it.getSnapDistanceX() == 0.0;
          }
        });
      }
