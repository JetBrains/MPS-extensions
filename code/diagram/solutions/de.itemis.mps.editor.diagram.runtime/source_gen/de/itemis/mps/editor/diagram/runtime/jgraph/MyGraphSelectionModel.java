...
public class MyGraphSelectionModel extends mxGraphSelectionModel {
  private static final Logger LOG = Logger.getLogger(MyGraphSelectionModel.class);
  ...
  protected void changeSelection(Collection<Object> added, Collection<Object> removed) {
    ...
    if (myDiagramCell != null && !(directionMpsToDiagram)) {
      ...
      if (selectedCells.size() > 0) {
        GraphSelection selection = new GraphSelection(myDiagramCell.getEditorComponent(), myDiagramCell, Sequence.fromIterable(Sequence.fromArray(getCells())).ofType(mxCell.class).toListSequence());
