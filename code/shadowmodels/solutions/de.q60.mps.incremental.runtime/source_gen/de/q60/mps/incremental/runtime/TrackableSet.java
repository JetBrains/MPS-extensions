...
public class TrackableSet<V> {
  ...
  public void replaceValues(Set<V> newValues) {
    List<V> changes = SetSequence.fromSet(values).disjunction(SetSequence.fromSet(newValues)).toListSequence();
    SetSequence.fromSet(values).clear();
    SetSequence.fromSet(values).addSequence(SetSequence.fromSet(newValues));
    DependencyBroadcaster.INSTANCE.dependenciesChanged(ListSequence.fromList(changes).select(new ISelector<V, SetContainsDependency>() {
      public SetContainsDependency select(V it) {
        return new SetContainsDependency(TrackableSet.this, it);
      }
    }));
  }
