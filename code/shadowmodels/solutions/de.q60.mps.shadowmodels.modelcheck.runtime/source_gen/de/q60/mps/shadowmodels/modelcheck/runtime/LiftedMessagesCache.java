...
public class LiftedMessagesCache {
  ...
  public synchronized List<LiftedReportItem> getMessages() {
    return Sequence.fromIterable(MapSequence.fromMap(messages).values()).translate(new ITranslator2<Map<SNodeReference, List<LiftedReportItem>>, List<LiftedReportItem>>() {
      public Iterable<List<LiftedReportItem>> translate(Map<SNodeReference, List<LiftedReportItem>> it) {
        return MapSequence.fromMap(it).values();
      }
    }).translate(new ITranslator2<List<LiftedReportItem>, LiftedReportItem>() {
      public Iterable<LiftedReportItem> translate(List<LiftedReportItem> it) {
        return it;
      }
    }).toListSequence();
  }
