...
public class TransformationResult implements ITransformationResult {
  ...
  public Iterable<Tuples._2<IUniqueContainmentTarget, IContainment>> getChildCalls() {
    return ListSequence.fromList(roots).ofType(TransformationResultChild.class).select(new ISelector<TransformationResultChild, Tuples._2<IUniqueContainmentTarget, IContainment>>() {
      public Tuples._2<IUniqueContainmentTarget, IContainment> select(TransformationResultChild it) {
        return MultiTuple.<IUniqueContainmentTarget,IContainment>from(it.call, (IContainment) null);
      }
    }).concat(Sequence.fromIterable(MapSequence.fromMap(allElements).values()).translate(new ITranslator2<ResultElement, Tuples._2<IUniqueContainmentTarget, IContainment>>() {
      public Iterable<Tuples._2<IUniqueContainmentTarget, IContainment>> translate(final ResultElement parent) {
        return ListSequence.fromList(parent.children).ofType(TransformationResultChild.class).select(new ISelector<TransformationResultChild, Tuples._2<IUniqueContainmentTarget, IContainment>>() {
          public Tuples._2<IUniqueContainmentTarget, IContainment> select(TransformationResultChild child) {
  ...
  public Iterable<IResultElement> getElements() {
    return Sequence.fromIterable(MapSequence.fromMap(allElements).values()).select(new ISelector<ResultElement, IResultElement>() {
      public IResultElement select(ResultElement it) {
  ...
  public Iterable<IOutputNodeReference> getRootNodes() {
    return ListSequence.fromList(roots).translate(new ITranslator2<Child, IOutputNodeReference>() {
      public Iterable<IOutputNodeReference> translate(Child it) {
  ...
  public IOutputNodeReference getRootNode(int index) {
    ...
    if (itr.hasNext()) {
      ...
      ShadowLogging.logError(msg, resultRef, "Found elements: ", Sequence.fromIterable(elements).select(new ISelector<IOutputNodeReference, INode>() {
        public INode select(IOutputNodeReference it) {
          return OutputNodeReferenceAsNode.create(engine, it);
        }
      }).toListSequence());
      throw new RuntimeException(msg);
    }
  ...
  public IResultElement getFirstRootElement(int index) {
    ...
    return ListSequence.fromList(roots).where(new IWhereFilter<Child>() {
      public boolean accept(Child it) {
        return Objects.equals(it.role, role);
      }
    }).ofType(ResultElementChild.class).select(new ISelector<ResultElementChild, ResultElement>() {
      public ResultElement select(ResultElementChild it) {
        return it.child;
      }
    }).first();
  }
  ...
  public Iterable<IOutputNodeReference> getRootNodes(int index) {
    ...
    }).translate(new ITranslator2<Child, IOutputNodeReference>() {
      public Iterable<IOutputNodeReference> translate(Child it) {
        return it.resolve(engine);
      }
    });
  ...
  public class ResultElement implements IResultElement {
    ...
    public Iterable<Child> getChildren(final String role) {
      return ListSequence.fromList(children).where(new IWhereFilter<Child>() {
        public boolean accept(Child it) {
    ...
    public void clearChildren(final String role) {
      checkNotFrozen();
      ListSequence.fromList(children).removeWhere(new IWhereFilter<Child>() {
        public boolean accept(Child it) {
          return Objects.equals(it.role.getName(), role);
        }
      });
