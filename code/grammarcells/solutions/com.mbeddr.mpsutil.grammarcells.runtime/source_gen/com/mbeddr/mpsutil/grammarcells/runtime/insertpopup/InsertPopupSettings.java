...
public class InsertPopupSettings {
  ...
  public void store() {
    PropertiesComponent.getInstance().setValue(RESOLVED_AMBIGUITIES_KEY, IterableUtils.join(MapSequence.fromMap(resolvedAmbiguities).select(new ISelector<IMapping<List<String>, Integer>, String>() {
      public String select(IMapping<List<String>, Integer> it) {
        return it.value() + ELEMENT_SEPARATOR + IterableUtils.join(ListSequence.fromList(it.key()), ELEMENT_SEPARATOR);
      }
    }), ENTRY_SEPARATOR));
  }
  ...
  public void load() {
    ...
    if ((serialized != null && serialized.length() > 0)) {
      for (String entry : Sequence.fromIterable(Sequence.fromArray(serialized.split(ENTRY_SEPARATOR))).where(new IWhereFilter<String>() {
        public boolean accept(String it) {
          return (it != null && it.length() > 0);
        }
      })) {
        List<String> elements = Sequence.fromIterable(Sequence.fromArray(entry.split(ELEMENT_SEPARATOR))).where(new IWhereFilter<String>() {
          public boolean accept(String it) {
            return (it != null && it.length() > 0);
          }
        ...
        MapSequence.fromMap(resolvedAmbiguities).put(ListSequence.fromList(elements).skip(1).toListSequence(), Integer.parseInt(ListSequence.fromList(elements).first()));
      }
    }
  }
