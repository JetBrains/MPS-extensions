...
public class AlignRightButton extends AlignButton {
  public AlignRightButton(double size, mxCellState state) {
    super(size, state);
  }
  public AlignRightButton(double size, mxCellState state, double spacing) {
  ...
  protected void paintSymbol(Graphics2D g) {
    ...
    g.draw(new Line2D.Double(ExtensionMethods.toPoint2D(bounds.getRightTop()), ExtensionMethods.toPoint2D(bounds.getRightBottom())));
  }

  @Override
  protected void doAlign(BaseDCell cell, BaseDCell otherCell) {
    Bounds bounds = CellExtensions.getAbsoluteBounds(otherCell);
    bounds.setRight(CellExtensions.getAbsoluteBounds(cell).getMaxX());
