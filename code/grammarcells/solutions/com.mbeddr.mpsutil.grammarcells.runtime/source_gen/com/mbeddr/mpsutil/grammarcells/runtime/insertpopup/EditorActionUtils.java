...
public class EditorActionUtils {
  ...
  private static void callAction(final EditorCell cell, final CellActionType cellAction, boolean after) {
    ...
    SetSequence.fromSet(actions).addSequence(ListSequence.fromList(sideTransformActions).select(new ISelector<SubstituteAction, CellActionExecuter>() {
      public CellActionExecuter select(SubstituteAction it) {
        return new CellActionExecuter(cell, new SubstituteActionAsCellAction(it), editorComponent.getEditorContext());
      }
    }));
    ...
    if (SetSequence.fromSet(actions).isEmpty()) {
      ...
      int resolvedChoice = InsertPopupSettings.INSTANCE.resolve(SetSequence.fromSet(actions).select(new ISelector<CellActionExecuter, String>() {
        public String select(CellActionExecuter it) {
          return it.toString();
        }
      }).toListSequence());
      if (resolvedChoice != -1) {
        ListSequence.fromList(SetSequence.fromSet(actions).toListSequence()).getElement(resolvedChoice).execute();
      } else {
        BaseListPopupStep<CellActionExecuter> popupStep = new InsertActionListPopupStep(SetSequence.fromSet(actions).toListSequence());
