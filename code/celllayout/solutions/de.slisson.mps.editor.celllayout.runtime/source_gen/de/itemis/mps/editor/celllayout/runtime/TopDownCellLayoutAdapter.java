...
public class TopDownCellLayoutAdapter extends AbstractCellLayout implements CellLayout {
  ...
  public int getAscent(final EditorCell_Collection collection) {
    ...
    LayoutWatchdog.enterIfRequired(LayoutWatchdog.DEFAULT_TIMEOUT, () -> ascent.value = getLayoutable(collection).getAscent());
    return ascent.value;
  }
  ...
  public List<Rectangle> getSelectionBounds(EditorCell_Collection collection) {
    return CellLayoutUtil.closeGaps(Sequence.fromIterable(myLayouter.getCellsForSelectionPainting(getLayoutable(collection))).select(new ISelector<ILayoutable, Rectangle>() {
      public Rectangle select(ILayoutable it) {
        return GeometryUtil.getBounds(CellLayoutUtil.getEditorCell(it));
      }
    }).toListSequence());
  }
  ...
  public List<? extends EditorCell> getSelectionCells(final EditorCell_Collection collection) {
    List<EditorCell> cells = Sequence.fromIterable(myLayouter.getCellsForSelectionPainting(getLayoutable(collection))).select(new ISelector<ILayoutable, EditorCell>() {
      public EditorCell select(ILayoutable it) {
        return CellLayoutUtil.getEditorCell(it);
      }
