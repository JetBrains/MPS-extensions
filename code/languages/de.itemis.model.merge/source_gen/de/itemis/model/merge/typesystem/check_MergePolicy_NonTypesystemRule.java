...
public class check_MergePolicy_NonTypesystemRule extends AbstractNonTypesystemRule_Runtime implements NonTypesystemRule_Runtime {
  ...
  public void applyRule(final SNode mergePolicy, final TypeCheckingContext typeCheckingContext, IsApplicableStatus status) {
    ...
    for (SNode childPolicy : ListSequence.fromList(SLinkOperations.getChildren(mergePolicy, LINKS.childPolicies$7ocQ)).where(new IWhereFilter<SNode>() {
      public boolean accept(SNode it) {
        return !(SNodeOperations.getConcept(it).isAbstract());
      }
    })) {
      if (SetSequence.fromSet(linkBuffer).contains(ItemPolicy__BehaviorDescriptor.childLink_id1VmHfRx_0K2.invoke(childPolicy))) {
        {
          final MessageTarget errorTarget = new NodeMessageTarget();
      ...
      SetSequence.fromSet(linkBuffer).addElement(ItemPolicy__BehaviorDescriptor.childLink_id1VmHfRx_0K2.invoke(childPolicy));
    }
  }
  public SAbstractConcept getApplicableConcept() {
    return CONCEPTS.MergePolicy$RB;
  }
