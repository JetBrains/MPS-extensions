...
**Language Namespace :** `io.f1re.testing`
...
- *void changeEditorSettings(EditorComponentSettingsImpl)* - change settings of the editor and rebuild the editor afterwards.
 ``` java title="Example: Increase the editor scaling (needs the reflection language)"
 new  EditorComponentTestHelper(editor component).changeEditorSettings({settings => settings.increaseUIScale(); })
 ```
...
- *void withClassLoading(action)* - access the class loader manager and execute some code
