...
public class LayoutDependencies {
  ...
  public static LayoutDependencies getInstance(EditorComponent editorComponent) {
    ...
    if (instance == null) {
      ...
      ((jetbrains.mps.nodeEditor.EditorComponent) editorComponent).addDisposeListener((jetbrains.mps.nodeEditor.EditorComponent e) -> MapSequence.fromMap(ourInstances).removeKey(e));
  ...
  public static void cleanInstances() {
    SetSequence.fromSet(MapSequence.fromMap(ourInstances).keySet()).where(new IWhereFilter<EditorComponent>() {
      public boolean accept(EditorComponent it) {
        return it.isDisposed();
      }
    }).toListSequence().visitAll(new IVisitor<EditorComponent>() {
      public void visit(EditorComponent it) {
        MapSequence.fromMap(ourInstances).removeKey(it);
      }
    });
  }
  ...
  public void cleanDependencies() {
    ...
    SetSequence.fromSet(layoutables).where(new IWhereFilter<Layoutable>() {
      public boolean accept(Layoutable it) {
        return !(it.isAlive());
      }
    }).toListSequence().visitAll(new IVisitor<Layoutable>() {
      public void visit(Layoutable it) {
  ...
  public void update() {
    ...
    List<Layoutable> invalidLayoutables = Sequence.fromIterable(entries).where(new IWhereFilter<Map.Entry<Layoutable, DependencyAndValue>>() {
      public boolean accept(Map.Entry<Layoutable, DependencyAndValue> it) {
        return it.getValue().isChanged();
      }
    }).select(new ISelector<Map.Entry<Layoutable, DependencyAndValue>, Layoutable>() {
      public Layoutable select(Map.Entry<Layoutable, DependencyAndValue> it) {
        return it.getKey();
      }
