...
public class DiagramActionsUtil {
  ...
  public static void fitSizeAll(MyGraph graph, boolean selectionOnly) {
    ...
    try {
      ...
      if (selectionOnly) {
        cells = Sequence.fromIterable(Sequence.fromArray(graph.getSelectionCells())).ofType(BaseDCell.class).toListSequence();
      } else {
        cells = ListSequence.fromList(ListSequence.fromListWithValues(new ArrayList(), model.getCells().values())).ofType(BaseDCell.class).toListSequence();
      }
  ...
  public static void toggleAllEdgesVisibility(MyGraph graph, @Nullable Object cell, final boolean visibility) {
    ...
    if (cell instanceof BoxDCell) {
      ...
      SetSequence.fromSet(edges).addSequence(Sequence.fromIterable(box.getModel().getEdges()).where(new IWhereFilter<Edge>() {
        public boolean accept(Edge it) {
          return EndpointUtil.getBox(it.getResolvedEndpointFrom()) == box || EndpointUtil.getBox(it.getResolvedEndpointTo()) == box;
        }
      }));

    }

    SetSequence.fromSet(edges).visitAll(new IVisitor<Edge>() {
      public void visit(Edge it) {
  ...
  public static void translateToOrigin(MyGraph graph, int offsetX, int offsetY) {
    ...
    Sequence.fromIterable(graphComponent.getDiagramCell().myModel.getBoxes()).visitAll(new IVisitor<Box>() {
      public void visit(Box it) {
    ...
    RootDiagramECell diagramCell = graphComponent.getDiagramCell();
    Sequence.fromIterable(diagramCell.myModel.getElements()).visitAll(new IVisitor<IDiagramElement>() {
