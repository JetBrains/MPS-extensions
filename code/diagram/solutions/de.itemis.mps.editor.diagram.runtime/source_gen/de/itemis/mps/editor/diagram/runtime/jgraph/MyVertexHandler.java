...
public class MyVertexHandler extends mxVertexHandler implements ISupportButtonHiding {
  ...
  protected void updateButtons() {
    ...
    if (buttonsCreator != null) {
      BoxBaseDCell boxCell = (BoxBaseDCell) state.getCell();
      EditorCell editorCell = boxCell.getBigCell();
  ...
  protected mxRectangle snapToSiblings(mxRectangle bounds, double dx, double dy, int index) {
    ...
    List<mxCellState> siblingStates = ListSequence.fromList(JGraphUtil.getSiblings(cell)).where(new IWhereFilter<mxCell>() {
      public boolean accept(mxCell it) {
        return it.getClass() == cell.getClass();
      }
    }).select(new ISelector<mxCell, mxCellState>() {
      public mxCellState select(mxCell it) {
        return view.getState(it);
      }
    }).toListSequence();
