...
public class TransformationEngineForEditor extends STransformationEngine {
    ...
    if (instance == null) {
      instance = new TransformationEngineForEditor(editor, new DefaultPFContext(ListSequence.fromListAndArray(new ArrayList<IImplementationProvider>(), new SM_TransformationsFromAspect() {
        ...
        protected Iterable<ReloadableModule> getSModules() {
          ...
          return SetSequence.fromSet(languages).select(new ISelector<SLanguage, ReloadableModule>() {
            public ReloadableModule select(SLanguage it) {
              return (ReloadableModule) it.getSourceModule();
            }
          });
        }
      })));
  ...
  public static void disposeAll() {
    Sequence.fromIterable(MapSequence.fromMap(instances).values()).visitAll(new IVisitor<TransformationEngineForEditor>() {
      public void visit(TransformationEngineForEditor it) {
        it.dispose();
      }
    });
    instances = null;
