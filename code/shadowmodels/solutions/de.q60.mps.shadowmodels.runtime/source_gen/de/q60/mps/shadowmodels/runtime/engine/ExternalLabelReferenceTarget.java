...
public class ExternalLabelReferenceTarget implements IReferenceTarget {
  ...
  public IOutputNodeReference resolve(IScope sourceScope, ITransformationEngine engine) {
    ...
    List<IOutputNodeReference> outputNodes = Sequence.fromIterable(stage.resolveContent(transformation)).select(new ISelector<IUniqueContainmentTarget, ITransformationResult>() {
      public ITransformationResult select(IUniqueContainmentTarget it) {
        return stage.getTransformationResult(it);
      }
    }).select(new ISelector<ITransformationResult, IResultElement>() {
      public IResultElement select(ITransformationResult it) {
        return it.getLabeledElement(label);
      }
    }).where(new NotNullWhereFilter<IResultElement>()).select(new ISelector<IResultElement, IOutputNodeReference>() {
      public IOutputNodeReference select(IResultElement it) {
        return it.getCanonicalReference();
      }
    }).toListSequence();
