...
public class IncrementalData {
  ...
  private Map<Object, CacheEntry> cacheEntries = new THashMap<Object, CacheEntry>();
  private Set<CacheEntry> invalidatedEntries = SetSequence.fromSet(new HashSet<CacheEntry>());
  ...
  public Set<Object> getDependingKeys(Iterable<DependencyKey> changes) {
    return SetSequence.fromSetWithValues(new HashSet<Object>(), Sequence.fromIterable(changes).translate(new ITranslator2<DependencyKey, CacheEntry>() {
      public Iterable<CacheEntry> translate(DependencyKey it) {
  ...
  public Set<Object> invalidate(Iterable<DependencyKey> changedDependencies) {
    try {
      ...
      invalidatedEntries = SetSequence.fromSet(new HashSet<CacheEntry>());
      for (DependencyKey change : Sequence.fromIterable(changedDependencies)) {
        if (change instanceof CachedValueDependency && ((CachedValueDependency) change).getEngine() == engine) {
          ...
          if (changedEntry != null) {
            Set<CacheEntry> incomingCacheDependencies = changedEntry.incomingCacheDependencies;
            for (CacheEntry cacheEntry : SetSequence.fromSet(incomingCacheDependencies)) {
      ...
      return SetSequence.fromSetWithValues(new HashSet<Object>(), SetSequence.fromSet(invalidatedEntries).select(new ISelector<CacheEntry, Object>() {
        public Object select(CacheEntry it) {
          return it.getKey();
        }
      }));
    } finally {
  ...
  public Set<CacheEntry> cleanup() {
    int oldSize = getSize();
    Set<CacheEntry> removedEntries = SetSequence.fromSet(new HashSet<CacheEntry>());

    Set<CacheEntry> queue = SetSequence.fromSet(new HashSet<CacheEntry>());
    Iterable<CacheEntry> allCacheEntries = cacheEntries.values();
    SetSequence.fromSet(queue).addSequence(Sequence.fromIterable(allCacheEntries).where(new IWhereFilter<CacheEntry>() {
      public boolean accept(CacheEntry it) {
    ...
    while (SetSequence.fromSet(queue).isNotEmpty()) {
      ...
      queue = SetSequence.fromSet(new HashSet<CacheEntry>());
      for (CacheEntry entry : SetSequence.fromSet(currentEntries).where(new IWhereFilter<CacheEntry>() {
        public boolean accept(CacheEntry it) {
          return !(it.isReachable());
        }
      })) {
  ...
  private void removeInvalidEntries(Object key) {
    ...
    Iterable<CacheEntry> invalidIncomingDeps = SetSequence.fromSet(entry.incomingCacheDependencies).where(new IWhereFilter<CacheEntry>() {
      public boolean accept(CacheEntry it) {
        return it.status == ECacheValueStatus.INVALID;
      }
    });
  ...
  public Set<Object> removeCacheEntry(CacheEntry entry) {
    Set<CacheEntry> removedKeys = SetSequence.fromSet(new HashSet<CacheEntry>());
    doRemoveCacheEntry(entry, removedKeys);
    return SetSequence.fromSetWithValues(new HashSet<Object>(), SetSequence.fromSet(removedKeys).select(new ISelector<CacheEntry, Object>() {
      public Object select(CacheEntry it) {
        return it.getKey();
      }
    }));
  ...
  private void doRemoveCacheEntry(final CacheEntry entry, Set<CacheEntry> removedEntries) {
    ...
    List<CacheEntry> incomingDependencies = SetSequence.fromSet(entry.incomingCacheDependencies).toListSequence();
    for (CacheEntry incomingDependency : ListSequence.fromList(incomingDependencies)) {
      doRemoveCacheEntry(incomingDependency, removedEntries);
    }
    if (SetSequence.fromSet(entry.incomingCacheDependencies).isNotEmpty()) {
      throw new IllegalStateException(entry + " still has entries depending on it: " + SetSequence.fromSet(entry.incomingCacheDependencies).select(new ISelector<CacheEntry, Object>() {
        public Object select(CacheEntry it) {
          return it.key;
        }
      }));
    }
    SetSequence.fromSet(entry.outgoingCacheDependencies).visitAll(new IVisitor<CacheEntry>() {
      public void visit(CacheEntry it) {
        it.removeIncomingDependency(entry);
      }
    });
    SetSequence.fromSet(entry.dependencies).visitAll(new IVisitor<DependencyKey>() {
      public void visit(DependencyKey it) {
        dependencies.remove(it, entry);
      }
    });
  ...
  public <T> void putEntry(Object key, T newValue, _FunctionTypes._return_P0_E0<? extends T> computable, Set<DependencyKey> dependencies) {
    CacheEntry<T> cacheEntry = cacheEntries.get(key);
    if (cacheEntry == null) {
    ...
    Set<CacheEntry> newOutgoingDeps = SetSequence.fromSet(new HashSet<CacheEntry>());
  ...
  public class CacheEntry<E> {
    ...
    private Set<CacheEntry> outgoingCacheDependencies = new THashSet<CacheEntry>();
    private Set<CacheEntry> incomingCacheDependencies = new THashSet<CacheEntry>();
    ...
    public boolean isStale() {
      if (isRoot) {
        ...
        return !(SetSequence.fromSet(incomingCacheDependencies).any(new IWhereFilter<CacheEntry>() {
          public boolean accept(CacheEntry it) {
            return !(it.isStale());
          }
        }));
    ...
    public void setStatus(ECacheValueStatus newStatus) {
      ...
      if (oldStatus == ECacheValueStatus.VALID) {
        ...
        SetSequence.fromSet(incomingCacheDependencies).visitAll(new IVisitor<CacheEntry>() {
          public void visit(CacheEntry it) {
    ...
    public void updateStatus() {
      if (status == ECacheValueStatus.DEPENDENCY_INVALID) {
        if (SetSequence.fromSet(outgoingCacheDependencies).all(new IWhereFilter<CacheEntry>() {
          public boolean accept(CacheEntry it) {
            return it.isValid();
          }
        })) {
    ...
    public List<CacheEntry> getInvalidDependencies() {
      return SetSequence.fromSet(outgoingCacheDependencies).where(new IWhereFilter<CacheEntry>() {
        public boolean accept(CacheEntry it) {
          return !(it.isValid());
        }
      }).toListSequence();
    ...
    protected void setValue(E newValue) {
      if (!(Objects.equals(newValue, getValue()))) {
        SetSequence.fromSet(incomingCacheDependencies).visitAll(new IVisitor<CacheEntry>() {
          public void visit(CacheEntry it) {
            it.setStatus(ECacheValueStatus.INVALID);
          }
        });
      }
      this.value = newValue;
    ...
    public void setOutgoingDependencies(Set<CacheEntry> deps) {
      List<CacheEntry> removedDeps = SetSequence.fromSet(outgoingCacheDependencies).subtract(SetSequence.fromSet(deps)).toListSequence();
      List<CacheEntry> addedDeps = SetSequence.fromSet(deps).subtract(SetSequence.fromSet(outgoingCacheDependencies)).toListSequence();
      ListSequence.fromList(removedDeps).visitAll(new IVisitor<CacheEntry>() {
        public void visit(CacheEntry it) {
          it.removeIncomingDependency(CacheEntry.this);
        }
      });
      ListSequence.fromList(addedDeps).visitAll(new IVisitor<CacheEntry>() {
        public void visit(CacheEntry it) {
          it.addIncomingDependency(CacheEntry.this);
        }
      });
