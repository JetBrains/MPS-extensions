...
public class GridElementFactory {
  ...
  public IGridElement createFromObjectList(List<Object> objects) {
    boolean allSNodes = ListSequence.fromList(((List<Object>) objects)).foldLeft(true, new ILeftCombinator<Object, Boolean>() {
      public Boolean combine(Boolean s, Object it) {
        return s && it instanceof SNode;
      }
    });
    if (allSNodes) {
  ...
  public IGridElement createFromSNodeList(List<SNode> snodes) {
    if (myNodeListAsMultipleCells) {
      return createFromGridElementList(new ArrayList<IGridElement>(ListSequence.fromList(snodes).select(new ISelector<SNode, IGridElement>() {
        public IGridElement select(SNode it) {
          return create(it);
        }
      }).toListSequence()));
    } else {
