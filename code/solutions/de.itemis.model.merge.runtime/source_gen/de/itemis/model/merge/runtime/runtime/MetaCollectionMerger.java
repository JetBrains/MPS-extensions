...
public class MetaCollectionMerger implements ConceptChildMerger {
  ...
  public List<SNode> merge(SNode leftParent, SNode rightParent, final Collection<SNode> leftChildren, Collection<SNode> rightChildren, _FunctionTypes._return_P2_E0<? extends SNode, ? super SNode, ? super SNode> recursiveMergeOperation) {
    ...
    for (final SNode leftElement : CollectionSequence.fromCollection(leftChildren)) {
      SNode equalRightElement = ListSequence.fromList(rightList).findFirst(new IWhereFilter<SNode>() {
        public boolean accept(SNode it) {
          return isSame(it, leftElement);
        }
      });
      ...
      if (noMatchYet) {
        ...
        SNode rightListFirstElemInLeft = ListSequence.fromList(rightList).findFirst(new IWhereFilter<SNode>() {
          public boolean accept(final SNode rcp) {
            return CollectionSequence.fromCollection(leftChildren).any(new IWhereFilter<SNode>() {
              public boolean accept(SNode lc) {
                return isSame(rcp, lc);
              }
            });
          }
        });
    ...
    ListSequence.fromList(resultPrefix).reversedList().visitAll(new IVisitor<SNode>() {
      public void visit(SNode it) {
        ListSequence.fromList(result).insertElement(0, it);
      }
    });
