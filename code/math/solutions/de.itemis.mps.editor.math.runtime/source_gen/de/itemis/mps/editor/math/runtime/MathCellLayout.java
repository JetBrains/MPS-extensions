...
public abstract class MathCellLayout extends AbstractCellLayout {
  ...
  public void doLayout(EditorCell_Collection collection) {
    ...
    if (showEndCells) {
      ListSequence.fromList(mathCell.getLayoutableChilds(false)).visitAll(new IVisitor<MathLayoutableCell>() {
        public void visit(MathLayoutableCell it) {
          it.setX(it.getXInt() + (leftEnd.value.getWidth() + 3));
        }
      });
    }
    int maxX = ListSequence.fromList(mathCell.getLayoutableChilds(false)).foldLeft(0, new ILeftCombinator<MathLayoutableCell, Integer>() {
      public Integer combine(Integer s, MathLayoutableCell it) {
        return Math.max(s, it.getXInt() + it.getWidthInt());
      }
    });
    ...
    Sequence.fromIterable(childCells).ofType(EditorCell_STHint.class).visitAll(new IVisitor<EditorCell_STHint>() {
      public void visit(EditorCell_STHint it) {
        it.moveTo(mathCell.getX(), mathCell.getY());
      }
    });
