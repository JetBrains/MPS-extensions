...
public abstract class SideTransformationHolderCell extends EditorCell_Constant {
  ...
  public static void processParentCollection(final EditorCell_Collection parentCollection) {
    ...
    for (SideTransformationHolderCell holderCell : ListSequence.fromList(children).ofType(SideTransformationHolderCell.class)) {
      ...
      nextSiblings = Sequence.fromIterable(nextSiblings).where(new IWhereFilter<EditorCell>() {
        public boolean accept(EditorCell it) {
          return !(ignoreSibling(it));
        }
      });
      prevSiblings = Sequence.fromIterable(prevSiblings).where(new IWhereFilter<EditorCell>() {
        public boolean accept(EditorCell it) {
          return !(ignoreSibling(it));
        }
      });
    ...
    ListSequence.fromList(children).ofType(SideTransformationHolderCell.class).visitAll(new IVisitor<SideTransformationHolderCell>() {
      public void visit(SideTransformationHolderCell it) {
        parentCollection.removeCell(it);
      }
    });
  }
      ...
      return Collections.<TransformationMenu>singleton(new TransformationMenu() {
        ...
        public List<TransformationMenuItem> createMenuItems(@NotNull final TransformationMenuContext context) {
          ...
          try {
            ...
            return ListSequence.fromList(createMenuParts()).translate(new ITranslator2<MenuPart<TransformationMenuItem, TransformationMenuContext>, TransformationMenuItem>() {
              public Iterable<TransformationMenuItem> translate(MenuPart<TransformationMenuItem, TransformationMenuContext> it) {
                return it.createItems(context.withNode(getSNode()));
              }
            }).toListSequence();
          } finally {
