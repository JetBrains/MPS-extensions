...
public class SNodeLoader implements SM_INodeLoader {
  ...
  public void updateChildren(SNode inputParent, long outputParent, SContainmentLink link, final SM_IWriteTransaction t, boolean updateDescendants) {
    ...
    List<SNodeReference> expectedOutputChildren = ListSequence.fromList(inputChildren).select(new ISelector<SNode, SNodeReference>() {
      public SNodeReference select(SNode it) {
        return it.getReference();
      }
    }).toListSequence();
    Iterable<Long> outputChildren = t.getChildren(outputParent, link.getName());
    List<SNodeReference> existingOutputChildren = Sequence.fromIterable(outputChildren).select(new ISelector<Long, SNodeReference>() {
      public SNodeReference select(Long it) {
        return (SNodeReference) t.getUserObject(it, SOURCE_NODE_REF_KEY);
      }
    }).toListSequence();
    ...
    if (!(Objects.equals(existingOutputChildren, expectedOutputChildren))) {
      ...
      Sequence.fromIterable(outputChildren2).skip(index).visitAll(new IVisitor<Long>() {
        public void visit(Long it) {
          t.moveChild(SM_PTree.ROOT_ID, DETACHED_ROLE, -1, it);
        }
      });
    }
