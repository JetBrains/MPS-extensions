...
public class OrthogonalTreeLayouter extends DefaultTreeLayouter {
  ...
  protected int getRootChildCenter(final boolean xAxis) {
    Iterable<IBounds> childrenBounds = Sequence.fromIterable(myTreeCell.getTreeChildren()).select(new ISelector<EditorCell, IBounds>() {
      public IBounds select(EditorCell it) {
        return getBounds(it, xAxis);
      }
    });
    IBounds rootBounds = getBounds(getTreeRoot(), xAxis);
    int childMin = Sequence.fromIterable(childrenBounds).select(new ISelector<IBounds, Integer>() {
      public Integer select(IBounds it) {
        return it.getY();
      }
