...
public class MappingLabelRefTarget implements IReferenceTarget {
  ...
  public IOutputNodeReference resolve(IScope sourceScope, final ITransformationEngine engine) {
    ...
    sourceScope.visitScopes((IScope scope) -> {
      ...
      List<IOutputNodeReference> outputNodes = Sequence.fromIterable(stage.getMappingLabels()).select(new ISelector<MappingLabels, IOutputNodesReference>() {
        public IOutputNodesReference select(MappingLabels it) {
          return it.getOutput(labelCall);
        }
      }).where(new NotNullWhereFilter<IOutputNodesReference>()).translate(new ITranslator2<IOutputNodesReference, IOutputNodeReference>() {
        public Iterable<IOutputNodeReference> translate(IOutputNodesReference it) {
