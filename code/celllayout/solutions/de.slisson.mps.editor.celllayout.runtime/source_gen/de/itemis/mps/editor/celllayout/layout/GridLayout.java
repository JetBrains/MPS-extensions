...
public class GridLayout extends AbstractGridLayout implements ILayouter {
  ...
  protected void layoutIntermediateCollection(ILayoutableContainer intermediateCollection) {
    if (ListSequence.fromList(intermediateCollection.getChildren()).isEmpty()) {
      ...
      for (ILayoutableContainer flattened : ListSequence.fromList(intermediateCollection.getChildren()).ofType(ILayoutableContainer.class).where(new IWhereFilter<ILayoutableContainer>() {
        public boolean accept(ILayoutableContainer it) {
          return isIntermediate(it);
        }
      })) {
