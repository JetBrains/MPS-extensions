...
public class RecursionProtection {
  ...
  public Iterable<IFunctionImplementation> filter(Iterable<IFunctionImplementation> implementations, final ParameterList parameters) {
    return Sequence.fromIterable(implementations).where(new IWhereFilter<IFunctionImplementation>() {
      public boolean accept(IFunctionImplementation it) {
        return !(SetSequence.fromSet(myActiveEvaluations.get()).contains(new Evaluation(it, parameters)));
      }
    }).toListSequence();
  }
