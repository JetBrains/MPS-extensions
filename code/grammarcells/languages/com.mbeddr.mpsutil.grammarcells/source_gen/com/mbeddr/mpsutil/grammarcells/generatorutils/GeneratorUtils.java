...
public class GeneratorUtils {
  ...
  public static SNode getCellToUnwrap(final SNode deletedCell) {
    ...
    Iterable<SNode> wrappedCells = ListSequence.fromList(wrapperCells).select(new ISelector<SNode, SNode>() {
      public SNode select(SNode it) {
        return SNodeOperations.as(ICellWrapper__BehaviorDescriptor.getWrappedLeaf_id3O7ZvCZLQaV.invoke(it), CONCEPTS.CellModel_RefNode$8);
      }
    }).where(new IWhereFilter<SNode>() {
      public boolean accept(SNode it) {
        return SLinkOperations.getTarget(it, LINKS.relationDeclaration$E2hc) != null;
      }
    });

    return Sequence.fromIterable(wrappedCells).findFirst(new IWhereFilter<SNode>() {
      public boolean accept(SNode it) {
        return SLinkOperations.getTarget(it, LINKS.relationDeclaration$E2hc) != SLinkOperations.getTarget(SNodeOperations.as(deletedCell, CONCEPTS.CellModel_RefNode$8), LINKS.relationDeclaration$E2hc);
      }
    });
  }
  ...
  public static boolean isBefore(SNode deletedCell, SNode unwrappedLink) {
    ...
    return ListSequence.fromList(leafsAfter).select(new ISelector<SNode, SNode>() {
      public SNode select(SNode it) {
        return SLinkOperations.getTarget(SNodeOperations.as(it, CONCEPTS.CellModel_RefNode$8), LINKS.relationDeclaration$E2hc);
      }
    }).contains(unwrappedLink);
  }
  ...
  public static boolean componentCellRequiresInline(SNode componentCell) {
    ...
    if (ListSequence.fromList(SNodeOperations.getNodeDescendants(rootCell, CONCEPTS.EditorCellModel$gN, true, new SAbstractConcept[]{})).any(new IWhereFilter<SNode>() {
      public boolean accept(SNode it) {
        return cellRequiresComponentInline(it);
      }
    })) {
      return true;
    }
  ...
  public static boolean componentRequiresInline(SNode componentDecl) {
    return ListSequence.fromList(SNodeOperations.getNodeDescendants(SLinkOperations.getTarget(componentDecl, LINKS.cellModel$L8Uc), CONCEPTS.EditorCellModel$gN, true, new SAbstractConcept[]{})).any(new IWhereFilter<SNode>() {
      public boolean accept(SNode it) {
        return cellRequiresComponentInline(it);
      }
    });
  ...
  public static SNode getRenderingCondition(SNode cell) {
    return ListSequence.fromList(SNodeOperations.getNodeAncestors(cell, CONCEPTS.EditorCellModel$gN, true)).select(new ISelector<SNode, SNode>() {
      public SNode select(SNode it) {
