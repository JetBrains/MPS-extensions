...
public class OutputNodeReferenceAsNode implements INode, INodeReference {
  ...
  public Iterable<INode> getChildren(final String role) {
    return Sequence.fromIterable(resolveNode().getChildren(role)).select(new ISelector<IOutputNodeReference, INode>() {
      public INode select(IOutputNodeReference it) {
        return wrapChild(it, role);
      }
    });
  }
  ...
  public Iterable<INode> getAllChildren() {
    ...
    return Sequence.fromIterable(childLinks).translate(new ITranslator2<IChildLink, INode>() {
      public Iterable<INode> translate(final IChildLink link) {
        return Sequence.fromIterable(n.getChildren(link.getName())).select(new ISelector<IOutputNodeReference, INode>() {
          public INode select(IOutputNodeReference child) {
            return wrapChild(child, link.getName());
          }
        });
      }
    });
  ...
  public List<String> getReferenceRoles() {
    ...
    return ListSequence.fromList(links).select(new ISelector<IReferenceLink, String>() {
