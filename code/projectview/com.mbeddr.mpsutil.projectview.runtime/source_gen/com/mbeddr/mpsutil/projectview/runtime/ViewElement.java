...
public class ViewElement<E> {
  ...
  public ViewElement(@Nullable E object, @Nullable ElementId descriptorId, Project project, @Nullable ViewElement parent, ViewId contextView) {
    ...
    if (myDescriptorId != null) {
      myFullHierachy = Sequence.fromIterable(ViewRegistry.getInstance().getInheritanceHierachy(myDescriptorId, contextView)).toListSequence();
      myInheritanceHierachy = ListSequence.fromList(myFullHierachy).where(new IWhereFilter<IViewElementDescriptor>() {
  ...
  public Object getObject(final ElementId id) {
    ...
    if (ListSequence.fromList(hierachy).any(new IWhereFilter<IViewElementDescriptor>() {
      public boolean accept(IViewElementDescriptor it) {
  ...
  public Set<Class> getPossibleDescendantTypes() {
    ...
    if (ListSequence.fromList(myInheritanceHierachy).isEmpty()) {
      ...
      result = SetSequence.fromSetWithValues(new HashSet<Class>(), ListSequence.fromList(myInheritanceHierachy).translate(new ITranslator2<IViewElementDescriptor, Class>() {
        public Iterable<Class> translate(IViewElementDescriptor it) {
  ...
  public Iterable<IViewElementDescriptor> getChildDescriptors() {
    ...
    final Set<ElementId> hierachy = SetSequence.fromSetWithValues(new HashSet<ElementId>(), Sequence.fromIterable(this.getInheritanceHierachy()).select(new ISelector<IViewElementDescriptor, ElementId>() {
